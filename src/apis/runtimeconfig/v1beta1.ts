      ...
      variables: {
        ...
        create(params, options, callback) {
          ...
          return createAPIRequest(parameters, callback);
            ...
            * variable name, then that variable is deleted. If you specify a
            * prefix and `recursive` is true, then all variables with that
        ...
        update(params, options, callback) {
          ...
          return createAPIRequest(parameters, callback);
            ...
            * variable's value. When there is a change, this method returns the
            * new value or times out.  If a variable is deleted while being
            * watched, the `variableState` state is set to `DELETED` and the
            * method returns the last known variable `value`.  If you set the
            * deadline for watching to a larger value than internal timeout (60
            * seconds), the current variable value is returned and the
            * `variableState` will be `VARIABLE_STATE_UNSPECIFIED`.  To learn
 ...
 * @memberOf! runtimeconfig(v1beta1)
 * @type object
 * @property {integer} number The number variables under the `path` that must exist to meet this condition. Defaults to 1 if not specified.
 ...
 * @memberOf! runtimeconfig(v1beta1)
 * @type object
 * @property {runtimeconfig(v1beta1).RuntimeConfig[]} configs A list of the configurations in the project. The order of returned objects is arbitrary; that is, it is not ordered in any particular way.
 * @property {string} nextPageToken This token allows you to get the next page of results for list requests. If the number of results is larger than `pageSize`, use the `nextPageToken` as a value for the query parameter `pageToken` in the next list request. Subsequent list requests will have their own `nextPageToken` to continue paging through the results
 */
/**
 ...
 * @type object
 * @property {string} nextPageToken This token allows you to get the next page of results for list requests. If the number of results is larger than `pageSize`, use the `nextPageToken` as a value for the query parameter `pageToken` in the next list request. Subsequent list requests will have their own `nextPageToken` to continue paging through the results
 * @property {runtimeconfig(v1beta1).Variable[]} variables A list of variables and their values. The order of returned variable objects is arbitrary.
 */
/**
 ...
 * @memberOf! runtimeconfig(v1beta1)
 * @type object
 * @property {runtimeconfig(v1beta1).Binding[]} bindings Associates a list of `members` to a `role`. `bindings` with no members will result in an error.
 ...
 * @property {string} timeout [Required] Specifies the timeout of the waiter in seconds, beginning from the instant that `waiters().create` method is called. If this time elapses before the success or failure conditions are met, the waiter fails and sets the `error` code to `DEADLINE_EXCEEDED`.
 */
/**
 * @typedef WatchVariableRequest
 * @memberOf! runtimeconfig(v1beta1)
 * @type object
 * @property {string} newerThan If specified, checks the current timestamp of the variable and if the current timestamp is newer than `newerThan` timestamp, the method returns immediately.  If not specified or the variable has an older timestamp, the watcher waits for a the value to change before returning.
 */
