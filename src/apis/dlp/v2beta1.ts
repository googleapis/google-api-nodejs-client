...
export namespace dlp_v2beta1 {
   ...
   * hashing. Uses SHA-256. Outputs a 32 byte digest as an uppercase hex string
   * (for example, 41D1567F7F99F1DC2A5FAB886DEE5BEE). Currently, only string and
   ...
   * set. For a given crypto key and context, the same identifier will be
   * replaced with the same surrogate. Identifiers must be at least two
  ...
  export interface Schema$GooglePrivacyDlpV2beta1CryptoReplaceFfxFpeConfig {
     ...
     * principle is that the likeliness is inversely related to the ratio of the
     * number of distinct identifiers per context over the number of possible
     * surrogates: As long as this ratio is small, the likehood is large.  If
     * the context is not set, a default tweak will be used. If the context is
     * set but:  1. there is no record present when transforming a given value
     * or 1. the field is not present when transforming a given value,  a
     ...
     * integer or string.  The tweak is constructed as a sequence of bytes in
     * big endian byte order such that:  - a 64 bit integer is encoded followed
     * by a single byte of value 1 - a string is encoded in UTF-8 format
     ...
     * name of custom info type is &#39;MY_TOKEN_INFO_TYPE&#39; and the
     * surrogate is &#39;abc&#39;, the full replacement value will be:
