...
function Firebaseremoteconfig(options) {
  ...
  self.projects = {
     ...
     */
    getRemoteConfig(params, options, callback) {
      ...
      options = options || {};
      const rootUrl =
          options.rootUrl || 'https://firebaseremoteconfig.googleapis.com/';
      const parameters = {
      ...
      return createAPIRequest(parameters, callback);
        ...
        * @desc Update a RemoteConfig. We treat this as an always-existing
        * resource (when it is not found in our data store, we treat it as
        * version 0, a template with zero conditions and zero parameters). Hence
        * there are no Create or Delete operations. Returns the updated template
        * when successful (and the updated eTag as a response header), or an
        * error if things go wrong. Possible error messages: * VALIDATION_ERROR
        * (HTTP status 400) with additional details if the template being passed
        * in can not be validated. * AUTHENTICATION_ERROR (HTTP status 401) if
        * the request can not be authenticate (e.g. no access token, or invalid
        * access token). * AUTHORIZATION_ERROR (HTTP status 403) if the request
        * can not be authorized (e.g. the user has no access to the specified
        * project id). * VERSION_MISMATCH (HTTP status 412) when trying to
        * update when the expected eTag (passed in via the "If-match" header) is
        * not specified, or is specified but does does not match the current
        * eTag. * Internal error (HTTP status 500) for Database problems or
        ...
        * @return {object} Request object
        */
    updateRemoteConfig(params, options, callback) {
      if (typeof options === 'function') {
        callback = options;
        options = {};
      }
 ...
 * @memberOf! firebaseremoteconfig(v1)
 * @type object
 * @property {object} conditionalValues Optional - a map of (condition_name, value). The condition_name of the highest priority (the one listed first in the conditions array) determines the value of this parameter.
 * @property {firebaseremoteconfig(v1).RemoteConfigParameterValue} defaultValue Optional - value to set the parameter to, when none of the named conditions evaluate to &lt;code&gt;true&lt;/code&gt;.
 * @property {string} description Optional. A description for this Parameter. Length must be less than or equal to 100 characters (or more precisely, unicode code points, which is defined in java/com/google/wireless/android/config/ConstsExporter.java). A description may contain any Unicode characters
 */
/**
