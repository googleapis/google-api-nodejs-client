...
export interface Schema$BandingProperties {
   ...
   * or column will be filled with either first_band_color or second_band_color,
   ...
   * or column. Otherwise, the first row or column will be filled with
   * first_band_color and the colors will proceed to alternate as they normally
   * would.
   */
...
export interface Schema$BasicChartSeries {
   ...
   * error to specify an axis that isn&#39;t a valid minor axis for the
   * chart&#39;s type.
   */
  targetAxis: string;
  /**
...
export interface Schema$BatchClearValuesByDataFilterResponse {
  /**
   * The ranges that were cleared, in A1 notation. (If the requests were for an
   * unbounded range or a ranger larger  than the bounds of the sheet, this will
   * be the actual ranges  that were cleared, bounded to the sheet&#39;s
   * limits.)
   */
...
export interface Schema$BatchUpdateValuesByDataFilterRequest {
   ...
   * matched by the specified DataFilter the specified values will be applied to
   * all of those ranges.
   */
   ...
   * BatchUpdateValuesResponse.responses will contain the updated values. If the
   * range to write was larger than than the range actually written, the
   * response will include all values in the requested range (excluding trailing
...
export interface Schema$BubbleChartSpec {
   ...
   * ID will be drawn in the same color. If bubble_sizes is specified then this
...
export interface Schema$CellData {
  ...
  /**
   * The effective value of the cell. For cells with formulas, this will be the
   * calculated value.  For cells with literals, this will be the same as the
   ...
   * runs.  When writing a new user_entered_value, previous runs will be erased.
   */
...
export interface Schema$ChartSpec {
   ...
   * labels, legend).  If a font is specified for a specific part of the chart
   * it will override this font name.
   */
  fontName: string;
  /**
...
export interface Schema$ConditionalFormatRule {
  /**
   * The formatting is either &quot;on&quot; or &quot;off&quot; according to the
   * rule.
   */
  ...
  /**
   * The ranges that will be formatted if the condition is true. All the ranges
...
export interface Schema$Editors {
   ...
   * protected range.  Domain protection is only supported on documents within a
   * domain.
   */
...
export interface Schema$FindReplaceRequest {
   ...
   * one cell has the contents `&quot;Google Sheets&quot;` and another has
   * `&quot;Google Docs&quot;`, then searching for `&quot;o.* (.*)&quot;` with a
   * replacement of `&quot;$1 Rocks&quot;` would change the contents of the
   * cells to `&quot;GSheets Rocks&quot;` and `&quot;GDocs Rocks&quot;`
   * respectively.
   */
...
export interface Schema$FindReplaceResponse {
   ...
   * example, if replacing `&quot;e&quot;` with `&quot;o&quot;` in `&quot;Google
   * Sheets&quot;`, this would be `&quot;3&quot;` because `&quot;Google
   * Sheets&quot;` -&gt; `&quot;Googlo Shoots&quot;`.
   */
 ...
 * HistogramRule.end will be placed into groups of size HistogramRule.interval.
 * In addition, all values below HistogramRule.start will be placed in one
 * group, and all values above HistogramRule.end will be placed in another. Only
 ...
 * chart&lt;/a&gt;. Org charts require a unique set of labels in labels and may
 ...
 * Sales (also reporting to Alice), have labels contain &quot;Alice&quot;,
 * &quot;Bob&quot;, &quot;Cathy&quot;, parent_labels contain &quot;&quot;,
 * &quot;Alice&quot;, &quot;Alice&quot; and tooltips contain &quot;CEO&quot;,
 * &quot;President&quot;, &quot;VP Sales&quot;.
 */
...
export interface Schema$PivotGroup {
   ...
   * minimize repitition of headings by not showing higher level headings where
   * they are the same. For example, even though the third row below corresponds
   * to &quot;Q1 Mar&quot;, &quot;Q1&quot; is not shown because it is redundant
   * with previous rows. Setting repeat_headings to true would cause
...
export interface Schema$PivotGroupSortValueBucket {
   ...
   * the first column group, and the second value corresponds to a value in the
   * second column group.  If no values are listed, this would indicate that the
   * row should be sorted according to the &quot;Grand Total&quot; over the
   * column groups. If a single value is listed, this would correspond to using
   * the &quot;Total&quot; of that bucket.
   */
...
export interface Schema$PivotTable {
  ...
  /**
   * An optional mapping of filters per source column offset.  The filters will
   * be applied before aggregating data into the pivot table. The map&#39;s key
...
export interface Schema$PivotValue {
   ...
   * pivot values will be displayed as the percentage of the grand total. In the
   * Sheets UI, this is referred to as &quot;Show As&quot; in the value section
   * of a pivot table.
   */
...
export interface Schema$SheetProperties {
   ...
   * sheet properties, if this field is excluded then the sheet will be added or
   ...
   * indexes. For example, if there were 3 sheets (S1, S2, S3) in order to move
   * S1 ahead of S2 the index would have to be set to 2. A sheet index update
   * request will be ignored if the requested index is identical to the sheets
   * current index or if the requested new index is equal to the current sheet
   * index + 1.
   */
  index: number;
  /**
...
export interface Schema$TextRotation {
   ...
   * characters is unchanged. For example:      | V |     | e |     | r |     |
   * t |     | i |     | c |     | a |     | l |
   */
  vertical: boolean;
}
...
/**
 * Updates a chart&#39;s specifications. (This does not move or resize a chart.
 * To move or resize a chart, use  UpdateEmbeddedObjectPositionRequest.)
 */
...
export interface Schema$UpdateEmbeddedObjectPositionRequest {
   ...
   * is implied and should not be specified. A single `&quot;*&quot;` can be
...
export class Resource$Spreadsheets {
   ...
   * request will fail and nothing will be applied.  Some requests have replies
   * to give you some information about how they are applied. The replies will
...
export class Resource$Spreadsheets$Developermetadata {
   ...
   * @desc Returns the developer metadata with the specified ID. The caller must
   ...
   * @desc Returns all developer metadata matching the specified DataFilter. If
...
export class Resource$Spreadsheets$Values {
   ...
   *   };
   *
   *   sheets.spreadsheets.values.batchClearByDataFilter(request, function(err,
   * response) { if (err) { console.error(err); return;
   *     }
   *
   *     // TODO: Change code below to process the `response` object:
   ...
   *   };
   *
   *   sheets.spreadsheets.values.batchUpdateByDataFilter(request, function(err,
   * response) { if (err) { console.error(err); return;
   *     }
   *
   *     // TODO: Change code below to process the `response` object:
   ...
   * @desc Returns a range of values from a spreadsheet. The caller must specify
   ...
   * @desc Sets values in a range of a spreadsheet. The caller must specify the
