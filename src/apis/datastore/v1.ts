 ...
 * @memberOf! datastore(v1)
 * @type object
 * @property {datastore(v1).Key[]} keys A list of keys with incomplete key paths for which to allocate IDs. No key may be reserved/read-only.
 */
/**
 * @typedef AllocateIdsResponse
 * @memberOf! datastore(v1)
 * @type object
 * @property {datastore(v1).Key[]} keys The keys specified in the request (in the same order), each with its key path completed with a newly allocated ID.
 */
/**
 * @typedef ArrayValue
 * @memberOf! datastore(v1)
 * @type object
 * @property {datastore(v1).Value[]} values Values in the array. The order of this array may not be preserved if it contains a mix of indexed and unindexed values.
 */
/**
 * @typedef BeginTransactionRequest
 * @memberOf! datastore(v1)
 * @type object
 * @property {datastore(v1).TransactionOptions} transactionOptions Options for a new transaction.
 */
/**
 ...
 * @property {datastore(v1).Mutation[]} mutations The mutations to perform.  When mode is `TRANSACTIONAL`, mutations affecting a single entity are applied in order. The following sequences of mutations affecting a single entity are not permitted in a single `Commit` request:  - `insert` followed by `insert` - `update` followed by `insert` - `upsert` followed by `insert` - `delete` followed by `update`  When mode is `NON_TRANSACTIONAL`, no two mutations may affect a single entity.
 ...
 * @property {datastore(v1).MutationResult[]} mutationResults The result of performing the mutations. The i-th mutation result corresponds to the i-th mutation in the request.
 */
/**
 * @typedef CompositeFilter
 * @memberOf! datastore(v1)
 * @type object
 * @property {datastore(v1).Filter[]} filters The list of filters to combine. Must contain at least one filter.
 ...
 * @memberOf! datastore(v1)
 * @type object
 * @property {datastore(v1).Key} key The entity&#39;s key.  An entity must have a key, unless otherwise documented (for example, an entity in `Value.entity_value` may have no key). An entity&#39;s kind is its key path&#39;s last element&#39;s kind, or null if it has no key.
 ...
 * @memberOf! datastore(v1)
 * @type object
 * @property {datastore(v1).GoogleDatastoreAdminV1beta1CommonMetadata} common Metadata common to all Datastore Admin operations.
 * @property {datastore(v1).GoogleDatastoreAdminV1beta1EntityFilter} entityFilter Description of which entities are being exported.
 ...
 * @property {datastore(v1).GoogleDatastoreAdminV1beta1Progress} progressBytes An estimate of the number of bytes processed.
 * @property {datastore(v1).GoogleDatastoreAdminV1beta1Progress} progressEntities An estimate of the number of entities processed.
 */
/**
 ...
 * @type object
 * @property {datastore(v1).GoogleDatastoreAdminV1beta1CommonMetadata} common Metadata common to all Datastore Admin operations.
 * @property {datastore(v1).GoogleDatastoreAdminV1beta1EntityFilter} entityFilter Description of which entities are being imported.
 ...
 * @property {datastore(v1).GoogleLongrunningOperation[]} operations A list of operations that matches the specified filter in the request.
 */
/**
 ...
 * @property {datastore(v1).Status} error The error result of the operation in case of failure or cancellation.
 ...
 * @property {datastore(v1).GqlQueryParameter[]} positionalBindings Numbered binding site @1 references the first numbered parameter, effectively using 1-based indexing, rather than the usual 0.  For each binding site numbered i in `query_string`, there must be an i-th numbered parameter. The inverse must also be true.
 ...
 * @memberOf! datastore(v1)
 * @type object
 * @property {datastore(v1).PartitionId} partitionId Entities are partitioned into subsets, currently identified by a project ID and namespace ID. Queries are scoped to a single partition.
 * @property {datastore(v1).PathElement[]} path The entity path. An entity path consists of one or more elements composed of a kind and a string or numerical identifier, which identify entities. The first element identifies a _root entity_, the second element identifies a _child_ of the root entity, the third element identifies a child of the second entity, and so forth. The entities identified by all prefixes of the path are called the element&#39;s _ancestors_.  An entity path is always fully complete: *all* of the entity&#39;s ancestors are required to be in the path along with the entity identifier itself. The only exception is that in some documented cases, the identifier in the last path element (for the entity) itself may be omitted. For example, the last path element of the key of `Mutation.insert` may have no identifier.  A path can never be empty, and a path can have at most 100 elements.
 */
/**
 ...
 * @memberOf! datastore(v1)
 * @type object
 * @property {datastore(v1).Key[]} deferred A list of keys that were not looked up due to resource constraints. The order of results in this field is undefined and has no relation to the order of the keys in the input.
 * @property {datastore(v1).EntityResult[]} found Entities found as `ResultType.FULL` entities. The order of results in this field is undefined and has no relation to the order of the keys in the input.
 * @property {datastore(v1).EntityResult[]} missing Entities not found as `ResultType.KEY_ONLY` entities. The order of results in this field is undefined and has no relation to the order of the keys in the input.
 */
/**
 * @typedef Mutation
 * @memberOf! datastore(v1)
 ...
 * @property {datastore(v1).Key} delete The key of the entity to delete. The entity may or may not already exist. Must have a complete key path and must not be reserved/read-only.
 * @property {datastore(v1).Entity} insert The entity to insert. The entity must not already exist. The entity key&#39;s final path element may be incomplete.
 * @property {datastore(v1).Entity} update The entity to update. The entity must already exist. Must have a complete key path.
 * @property {datastore(v1).Entity} upsert The entity to upsert. The entity may or may not already exist. The entity key&#39;s final path element may be incomplete.
 */
/**
 ...
 * @property {datastore(v1).Key} key The automatically allocated key. Set only when the mutation allocated a key.
 ...
 * @memberOf! datastore(v1)
 * @type object
 * @property {datastore(v1).PropertyReference[]} distinctOn The properties to make distinct. The query results will contain the first result for each distinct combination of values for the given properties (if empty, all results are returned).
 ...
 * @property {datastore(v1).KindExpression[]} kind The kinds to query (if empty, returns entities of all kinds). Currently at most 1 kind may be specified.
 ...
 * @property {datastore(v1).Key[]} keys A list of keys with complete key paths whose numeric IDs should not be auto-allocated.
 */
/**
 ...
 * @property {datastore(v1).PartitionId} partitionId Entities are partitioned into subsets, identified by a partition ID. Queries are scoped to a single partition. This partition ID is normalized with the standard default context partition ID.
 ...
 * @memberOf! datastore(v1)
 * @type object
 * @property {datastore(v1).ReadOnly} readOnly The transaction should only allow reads.
 * @property {datastore(v1).ReadWrite} readWrite The transaction should allow both reads and writes.
 */
/**
 * @typedef Value
 * @memberOf! datastore(v1)
 * @type object
 * @property {datastore(v1).ArrayValue} arrayValue An array value. Cannot contain another array value. A `Value` instance that sets field `array_value` must not set fields `meaning` or `exclude_from_indexes`.
 ...
 * @property {datastore(v1).Entity} entityValue An entity value.  - May have no key. - May have a key with an incomplete key path. - May have a reserved/read-only key.
