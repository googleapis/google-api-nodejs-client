...
export namespace remotebuildexecution_v2 {
  ...
  export class Resource$Actionresults {
     ...
     * @desc Upload a new execution result.  This method is intended for servers
     * which implement the distributed cache independently of the Execution API.
     * As a result, it is OPTIONAL for servers to implement.  In order to allow
     * the server to perform access control based on the type of action, and to
     ...
     * `ContentAddressableStorage`.  Errors: * `UNIMPLEMENTED`: This method is
     * not supported by the server. * `RESOURCE_EXHAUSTED`: There is
  ...
  export class Resource$Actions {
     ...
     * `DEADLINE_EXCEEDED`: The execution timed out.  In the case of a missing
     * input or command, the server SHOULD additionally send a
     * PreconditionFailure error detail where, for each requested blob not
     * present in the CAS, there is a `Violation` with a `type` of `MISSING` and
     * a `subject` of `"blobs/{hash}/{size}"` indicating the digest of the
     * missing blob.
