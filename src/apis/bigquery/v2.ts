 ...
 * @property {string} waitMsMax Milliseconds the slowest shard spent waiting to be scheduled.
 ...
 * @property {string} writeMsAvg Milliseconds the average shard spent on writing output.
 * @property {string} writeMsMax Milliseconds the slowest shard spent on writing output.
 ...
 * @property {string} nullMarker [Optional] Specifies a string that represents a null value in a CSV file. For example, if you specify &quot;x/&quot;, BigQuery interprets &quot;x/&quot; as a null value when loading a CSV file. The default value is the empty string. If you set this property to a custom value, BigQuery throws an error if an empty string is present for all data types except for STRING and BYTE. For STRING and BYTE columns, BigQuery interprets the empty string as an empty value.
 ...
 * @property {string[]} schemaUpdateOptions Allows the schema of the destination table to be updated as a side effect of the query job. Schema update options are supported in two cases: when writeDisposition is WRITE_APPEND; when writeDisposition is WRITE_TRUNCATE and the destination table is a partition of a table, specified by partition decorators. For normal tables, WRITE_TRUNCATE will always overwrite the schema. One or more of the following values are specified: ALLOW_FIELD_ADDITION: allow adding a nullable field to the schema. ALLOW_FIELD_RELAXATION: allow relaxing a required field in the original schema to nullable.
 ...
 * @memberOf! bigquery(v2)
 * @type object
 * @property {string} badRecords [Output-only] The number of bad records encountered. Note that if the job has failed because of more bad records encountered than the maximum allowed in the load job configuration, then this number can be less than the total number of bad records present in the input data.
 ...
 * @property {string} kind [Output-only] The type of the resource.
 * @property {object} labels The labels associated with this table. You can use these to organize and group your tables. Label keys and values can be no longer than 63 characters, can only contain lowercase letters, numeric characters, underscores and dashes. International characters are allowed. Label values are optional. Label keys must start with a letter and each label in the list must have a different key.
 ...
 * @property {bigquery(v2).TimePartitioning} timePartitioning If specified, configures time-based partitioning for this table.
