...
export interface Schema$AlertPolicy {
   ...
   * ListNotificationChannels method. The syntax of the entries in this field
...
export interface Schema$Condition {
   ...
   * the condition identifiers and includes them in the new policy.When calling
   * the alertPolicies.update method to update a policy, including a condition
   * name causes the existing condition to be updated. Conditions without names
   * are added to the updated policy. Existing conditions are deleted if they
   * are not updated.Best practice is to preserve [CONDITION_ID] if you make
   * only small changes, such as those to condition thresholds, durations, or
...
export interface Schema$GetNotificationChannelVerificationCodeRequest {
   ...
   * specifying an expiration may extend the code&#39;s lifetime over omitting
   * an expiration, even though the API does impose an upper limit on the
 ...
 * metadata. If a group&#39;s filter matches an available monitored resource,
 * then that resource is a member of that group. Groups can contain any number
 * of monitored resources, and each monitored resource can be a member of any
 ...
 * running a set of instances with two user-defined tags:
 ...
 * configuration defines a metric and a set of monitored resources. The
...
export interface Schema$MetricAbsence {
   ...
   * time series that will be retrieved / processed) and must specify the metric
...
export interface Schema$NotificationChannel {
   ...
   * projects/[PROJECT_ID]/notificationChannels/[CHANNEL_ID] The [CHANNEL_ID] is
...
export interface Schema$TimeSeries {
   ...
   * value type of the associated metric. If the associated metric&#39;s
...
export interface Schema$UptimeCheckConfig {
   ...
   * response&#39;s content. This field is optional and should only be specified
   * if a content match is required.
   */
...
export class Resource$Projects$Alertpolicies {
   ...
   * @desc Updates an alerting policy. You can either replace the entire policy
   * with a new one or replace only certain fields in the current alerting
   * policy by specifying the fields to be updated via updateMask. Returns the
...
export class Resource$Projects$Collectdtimeseries {
   ...
   * `"projects/PROJECT_ID_OR_NUMBER"`. name: "projects/{MY-PROJECT}", resource:
   * {},
   *     // Auth client
   *     auth: authClient
   *   };
...
export class Resource$Projects$Groups {
   ...
   * @desc Updates an existing group. You can change any group attributes except
   * name.
   * @example
   * * // PRE-REQUISITES:
   * // ---------------
...
export class Resource$Projects$Metricdescriptors {
   ...
   * @desc Deletes a metric descriptor. Only user-created custom metrics can be
   * deleted.
   * @example
   * * // PRE-REQUISITES:
   * // ---------------
...
export class Resource$Projects$Monitoredresourcedescriptors {
   ...
   * @desc Lists monitored resource descriptors that match a filter. This method
...
export class Resource$Projects$Notificationchannels {
   ...
   * @desc Gets a single notification channel. The channel includes the relevant
   ...
   * @desc Requests a verification code for an already verified channel that can
   * then be used in a call to VerifyNotificationChannel() on a different
   * channel with an equivalent identity in the same or in a different project.
   * This makes it possible to copy a channel between projects without requiring
...
export class Resource$Projects$Timeseries {
   ...
   * @desc Lists time series that match a filter. This method does not require a
   * Stackdriver account.
   * @example
   * * // PRE-REQUISITES:
   * // ---------------
...
export class Resource$Projects$Uptimecheckconfigs {
   ...
   * @desc Lists the existing valid uptime check configurations for the project,
