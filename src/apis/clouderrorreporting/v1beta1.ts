    ...
    events: {
      ...
      list(params, options, callback) {
        ...
        return createAPIRequest(parameters, callback);
          ...
          * href="https://support.google.com/cloud/answer/6158862">API key</a>
          * for authentication. To use an API key, append it to the URL as the
          * value of a `key` parameter. For example: <pre>POST
 ...
 * @memberOf! clouderrorreporting(v1beta1)
 * @type object
 * @property {clouderrorreporting(v1beta1).HttpRequestContext} httpRequest The HTTP request which was processed when the error was triggered.
 * @property {clouderrorreporting(v1beta1).SourceLocation} reportLocation The location in the source code where the decision was made to report the error, usually the place where it was logged. For a logged exception this would be the source line where the exception is logged, usually close to the place where it was caught.
 * @property {clouderrorreporting(v1beta1).SourceReference[]} sourceReferences Source code that was used to build the executable which has caused the given error message.
 * @property {string} user The user who caused or was affected by the crash. This can be a user ID, an email address, or an arbitrary token that uniquely identifies the user. When sending an error report, leave this field empty if the user was not logged in. In this case the Error Reporting system will use other data, such as remote IP address, to distinguish affected users. See `affected_users_count` in `ErrorGroupStats`.
 */
/**
 * @typedef ErrorEvent
 * @memberOf! clouderrorreporting(v1beta1)
 ...
 * @property {string} eventTime Time when the event occurred as provided in the error report. If the report did not contain a timestamp, the time the error was received by the Error Reporting system is used.
 ...
 * @memberOf! clouderrorreporting(v1beta1)
 * @type object
 * @property {string} groupId Group IDs are unique for a given project. If the same kind of error occurs in different service contexts, it will receive the same group ID.
 * @property {string} name The group resource name. Example: &lt;code&gt;projects/my-project-123/groups/my-groupid&lt;/code&gt;
 ...
 * @memberOf! clouderrorreporting(v1beta1)
 * @type object
 * @property {clouderrorreporting(v1beta1).ServiceContext[]} affectedServices Service contexts with a non-zero error count for the given filter criteria. This list can be truncated if multiple services are affected. Refer to `num_affected_services` for the total count.
 * @property {string} affectedUsersCount Approximate number of affected users in the given group that match the filter criteria. Users are distinguished by data in the `ErrorContext` of the individual error events, such as their login name or their remote IP address in case of HTTP requests. The number of affected users can be zero even if the number of errors is non-zero if no data was provided from which the affected user could be deduced. Users are counted based on data in the request context that was provided in the error report. If more users are implicitly affected, such as due to a crash of the whole service, this is not reflected here.
 * @property {string} count Approximate total number of events in the given group that match the filter criteria.
 * @property {string} firstSeenTime Approximate first occurrence that was ever seen for this group and which matches the given filter criteria, ignoring the time_range that was specified in the request.
 * @property {clouderrorreporting(v1beta1).ErrorGroup} group Group data that is independent of the filter criteria.
 * @property {string} lastSeenTime Approximate last occurrence that was ever seen for this group and which matches the given filter criteria, ignoring the time_range that was specified in the request.
 * @property {integer} numAffectedServices The total number of services with a non-zero error count for the given filter criteria.
 * @property {clouderrorreporting(v1beta1).ErrorEvent} representative An arbitrary event that is chosen as representative for the whole group. The representative event is intended to be used as a quick preview for the whole group. Events in the group are usually sufficiently similar to each other such that showing an arbitrary representative provides insight into the characteristics of the group as a whole.
 * @property {clouderrorreporting(v1beta1).TimedCount[]} timedCounts Approximate number of occurrences over time. Timed counts returned by ListGroups are guaranteed to be:  - Inside the requested time interval - Non-overlapping, and - Ordered by ascending time.
 */
/**
 ...
 * @property {string} remoteIp The IP address from which the request originated. This can be IPv4, IPv6, or a token which is derived from the IP address, depending on the data that has been provided in the error report.
 ...
 * @property {string} nextPageToken If non-empty, more results are available. Pass this token, along with the same query parameters as the first request, to view the next page of results.
 ...
 * @property {string} nextPageToken If non-empty, more results are available. Pass this token, along with the same query parameters as the first request, to view the next page of results.
 * @property {string} timeRangeBegin The timestamp specifies the start time to which the request was restricted. The start time is set based on the requested time range. It may be adjusted to a later time if a project has exceeded the storage quota and older data has been deleted.
 */
/**
 ...
 * @memberOf! clouderrorreporting(v1beta1)
 * @type object
 * @property {string} resourceType Type of the MonitoredResource. List of possible values: https://cloud.google.com/monitoring/api/resources  Value is set automatically for incoming errors and must not be set when reporting errors.
 * @property {string} service An identifier of the service, such as the name of the executable, job, or Google App Engine service name. This field is expected to have a low number of values that are relatively stable over time, as opposed to `version`, which can be changed whenever new code is deployed.  Contains the service name for error reports extracted from Google App Engine logs or `default` if the App Engine default service is used.
 * @property {string} version Represents the source code version that the developer provided, which could represent a version label or a Git SHA-1 hash, for example. For App Engine standard environment, the version is set to the version of the app.
 */
/**
 * @typedef SourceLocation
 * @memberOf! clouderrorreporting(v1beta1)
 * @type object
 * @property {string} filePath The source code filename, which can include a truncated relative path, or a full path from a production machine.
 * @property {string} functionName Human-readable name of a function or method. The value can include optional context like the class or package name. For example, `my.package.MyClass.method` in case of Java.
