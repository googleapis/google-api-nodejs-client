  ...
  self.beacons = {
    ...
    register(params, options, callback) {
      ...
      return createAPIRequest(parameters, callback);
        ...
        * this method will be  silently ignored. To update beacon status, use
        * the separate methods on this API for activation, deactivation, and
 ...
 * @property {number} maxDistanceMeters The distance away from the beacon at which this attachment should be delivered to a mobile app.  Setting this to a value greater than zero indicates that the app should behave as if the beacon is &quot;seen&quot; when the mobile device is less than this distance away from the beacon.  Different attachments on the same beacon can have different max distances.  Note that even though this value is expressed with fractional meter precision, real-world behavior is likley to be much less precise than one meter, due to the nature of current Bluetooth radio technology.  Optional. When not set or zero, the attachment should be delivered at the beacon&#39;s outer limit of detection.
 * @property {string} namespacedType Specifies what kind of attachment this is. Tells a client how to interpret the `data` field. Format is &lt;var&gt;namespace/type&lt;/var&gt;, for example &lt;code&gt;scrupulous-wombat-12345/welcome-message&lt;/code&gt;
 */
/**
 * @typedef Beacon
 * @memberOf! proximitybeacon(v1beta1)
 ...
 * @memberOf! proximitybeacon(v1beta1)
 * @type object
 * @property {string} attachmentName Resource name of this attachment. Attachment names have the format: &lt;code&gt;beacons/&lt;var&gt;beacon_id&lt;/var&gt;/attachments/&lt;var&gt;attachment_id&lt;/var&gt;&lt;/code&gt;. Leave this empty on creation.
 * @property {string} creationTimeMs The UTC time when this attachment was created, in milliseconds since the UNIX epoch.
 ...
 * @property {string} namespacedType Specifies what kind of attachment this is. Tells a client how to interpret the `data` field. Format is &lt;var&gt;namespace/type&lt;/var&gt;. Namespace provides type separation between clients. Type describes the type of `data`, for use by the client when parsing the `data` field. Required.
 */
/**
 * @typedef BeaconInfo
 * @memberOf! proximitybeacon(v1beta1)
 ...
 * @property {proximitybeacon(v1beta1).AttachmentInfo[]} attachments Attachments matching the type(s) requested. May be empty if no attachment types were requested.
 ...
 * @memberOf! proximitybeacon(v1beta1)
 * @type object
 * @property {string} namespaceName Resource name of this namespace. Namespaces names have the format: &lt;code&gt;namespaces/&lt;var&gt;namespace&lt;/var&gt;&lt;/code&gt;.
 ...
 * @memberOf! proximitybeacon(v1beta1)
 * @type object
 * @property {proximitybeacon(v1beta1).AdvertisedId} advertisedId The ID advertised by the beacon the client has encountered.  If the submitted `advertised_id` type is Eddystone-EID, then the client must be authorized to resolve the given beacon. Otherwise no data will be returned for that beacon. Required.
