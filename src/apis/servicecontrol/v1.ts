  ...
  self.services = {
    ...
    releaseQuota(params, options, callback) {
      ...
      return createAPIRequest(parameters, callback);
        ...
        * logs and metrics. It should be called after an operation is completed.
        * If feasible, the client should aggregate reporting data for up to 5
        * seconds to reduce API traffic. Limiting aggregation to 5 seconds is to
        * reduce data loss during client crashes. Clients should carefully
        * choose the aggregation time window to avoid data loss risk more than
        * 0.01% for business and compliance reasons.  NOTE: the ReportRequest
        * has the size limit of 1MB.  This method requires the
    ...
    report(params, options, callback) {
      ...
      return createAPIRequest(parameters, callback);
        ...
        * service differs from what the One Platform considers the usage is.
        * This is expected to happen only rarely, but over time this can
        * accumulate. Services can invoke StartReconciliation and
        * EndReconciliation to correct this usage drift, as described below: 1.
        * Service sends StartReconciliation with a timestamp in future for each
        * metric that needs to be reconciled. The timestamp being in future
        * allows    to account for in-flight AllocateQuota and ReleaseQuota
        * requests for the    same metric. 2. One Platform records this
        * timestamp and starts tracking subsequent    AllocateQuota and
        * ReleaseQuota requests until EndReconciliation is    called. 3. At or
        * after the time specified in the StartReconciliation, service    sends
        * EndReconciliation with the usage that needs to be reconciled to. 4.
        * One Platform adjusts its own record of usage for that metric to the
        * value specified in EndReconciliation by taking in to account any
        * allocation or release between StartReconciliation and
        * EndReconciliation.  Signals the quota controller that the service
        * wants to perform a usage reconciliation as specified in the request.
        * This method requires the `servicemanagement.services.quota` permission
        * on the specified service. For more information, see [Google Cloud
        * IAM](https://cloud.google.com/iam).
 ...
 * @memberOf! servicecontrol(v1)
 * @type object
 * @property {string[]} unusedArguments A list of label keys that were unused by the server in processing the request. Thus, for similar requests repeated in a certain future time window, the caller can choose to ignore these labels in the requests to achieve better client-side cache hits and quota aggregation.
 */
/**
 ...
 * @property {string} serviceConfigId Specifies which version of service configuration should be used to process the request. If unspecified or no matching version can be found, the latest one will be used.
 */
/**
 ...
 * @property {string} operationId The same operation_id value used in the AllocateQuotaRequest. Used for logging and diagnostics purposes.
 * @property {servicecontrol(v1).MetricValueSet[]} quotaMetrics Quota metrics to indicate the result of allocation. Depending on the request, one or more of the following metrics will be included:  1. Per quota group or per quota metric incremental usage will be specified using the following delta metric :   &quot;serviceruntime.googleapis.com/api/consumer/quota_used_count&quot;  2. The quota limit reached condition will be specified using the following boolean metric :   &quot;serviceruntime.googleapis.com/quota/exceeded&quot;
 ...
 * @property {servicecontrol(v1).AuthorizationInfo[]} authorizationInfo Authorization information. If there are multiple resources or permissions involved, then there is one AuthorizationInfo element for each {resource, permission} tuple.
 * @property {object} metadata Other service-specific data about the request, response, and other information associated with the current audited event.
 * @property {string} methodName The name of the service method or operation. For API calls, this should be the name of the API method. For example,      &quot;google.datastore.v1.Datastore.RunQuery&quot;     &quot;google.logging.v1.LoggingService.DeleteLog&quot;
 * @property {string} numResponseItems The number of items returned from a List or Query API method, if applicable.
 * @property {object} request The operation request. This may not include all request parameters, such as those that are too large, privacy-sensitive, or duplicated elsewhere in the log record. It should never include user-generated data, such as file contents. When the JSON object represented here has a proto equivalent, the proto name will be indicated in the `@type` property.
 * @property {servicecontrol(v1).RequestMetadata} requestMetadata Metadata about the operation.
 * @property {string} resourceName The resource or collection that is the target of the operation. The name is a scheme-less URI, not including the API service name. For example:      &quot;shelves/SHELF_ID/books&quot;     &quot;shelves/SHELF_ID/books/BOOK_ID&quot;
 * @property {object} response The operation response. This may not include all response elements, such as those that are too large, privacy-sensitive, or duplicated elsewhere in the log record. It should never include user-generated data, such as file contents. When the JSON object represented here has a proto equivalent, the proto name will be indicated in the `@type` property.
 * @property {object} serviceData Deprecated, use `metadata` field instead. Other service-specific data about the request, response, and other activities.
 * @property {string} serviceName The name of the API service performing the operation. For example, `&quot;datastore.googleapis.com&quot;`.
 ...
 * @memberOf! servicecontrol(v1)
 * @type object
 * @property {string} authoritySelector The authority selector specified by the requestor, if any. It is not guaranteed that the principal was allowed to use this authority.
 * @property {string} principalEmail The email address of the authenticated user (or service account on behalf of third party principal) making the request. For privacy reasons, the principal email address is redacted for all read-only operations that fail with a &quot;permission denied&quot; error.
 * @property {object} thirdPartyPrincipal The third party identification (if any) of the authenticated user making the request. When the JSON object represented here has a proto equivalent, the proto name will be indicated in the `@type` property.
 */
/**
 * @typedef AuthorizationInfo
 * @memberOf! servicecontrol(v1)
 * @type object
 * @property {boolean} granted Whether or not authorization for `resource` and `permission` was granted.
 * @property {string} permission The required IAM permission.
 * @property {string} resource The resource being accessed, as a REST-style string. For example:      bigquery.googleapis.com/projects/PROJECTID/datasets/DATASETID
 */
/**
 * @typedef CheckError
 * @memberOf! servicecontrol(v1)
 ...
 * @property {string[]} unusedArguments A list of fields and label keys that are ignored by the server. The client doesn&#39;t need to send them for following requests to improve performance and allow better aggregation.
 */
/**
 ...
 * @property {string} serviceConfigId Specifies which version of service configuration should be used to process the request.  If unspecified or no matching version can be found, the latest one will be used.
 * @property {boolean} skipActivationCheck Indicates if service activation check should be skipped for this request. Default behavior is to perform the check and apply relevant quota.
 */
/**
 * @typedef CheckResponse
 * @memberOf! servicecontrol(v1)
 * @type object
 * @property {servicecontrol(v1).CheckError[]} checkErrors Indicate the decision of the check.  If no check errors are present, the service should process the operation. Otherwise the service should use the list of errors to determine the appropriate action.
 * @property {servicecontrol(v1).CheckInfo} checkInfo Feedback data returned from the server during processing a Check request.
 * @property {string} operationId The same operation_id value used in the CheckRequest. Used for logging and diagnostics purposes.
 ...
 * @memberOf! servicecontrol(v1)
 * @type object
 * @property {string} projectNumber The Google cloud project number, e.g. 1234567890. A value of 0 indicates no project number is found.
 */
/**
 * @typedef Distribution
 * @memberOf! servicecontrol(v1)
 * @type object
 * @property {string[]} bucketCounts The number of samples in each histogram bucket. `bucket_counts` are optional. If present, they must sum to the `count` value.  The buckets are defined below in `bucket_option`. There are N buckets. `bucket_counts[0]` is the number of samples in the underflow bucket. `bucket_counts[1]` to `bucket_counts[N-1]` are the numbers of samples in each of the finite buckets. And `bucket_counts[N] is the number of samples in the overflow bucket. See the comments of `bucket_option` below for more details.  Any suffix of trailing zeros may be omitted.
 ...
 * @property {number} mean The arithmetic mean of the samples in the distribution. If `count` is zero then this field must be zero.
 * @property {number} minimum The minimum of the population of values. Ignored if `count` is zero.
 * @property {number} sumOfSquaredDeviation The sum of squared deviations from the mean:   Sum[i=1..count]((x_i - mean)^2) where each x_i is a sample values. If `count` is zero then this field must be zero, otherwise validation of the request fails.
 */
/**
 ...
 * @memberOf! servicecontrol(v1)
 * @type object
 * @property {number} growthFactor The i&#39;th exponential bucket covers the interval   [scale * growth_factor^(i-1), scale * growth_factor^i) where i ranges from 1 to num_finite_buckets inclusive. Must be larger than 1.0.
 * @property {integer} numFiniteBuckets The number of finite buckets. With the underflow and overflow buckets, the total number of buckets is `num_finite_buckets` + 2. See comments on `bucket_options` for details.
 * @property {number} scale The i&#39;th exponential bucket covers the interval   [scale * growth_factor^(i-1), scale * growth_factor^i) where i ranges from 1 to num_finite_buckets inclusive. Must be &gt; 0.
 */
/**
 ...
 * @type object
 * @property {integer} numFiniteBuckets The number of finite buckets. With the underflow and overflow buckets, the total number of buckets is `num_finite_buckets` + 2. See comments on `bucket_options` for details.
 * @property {number} offset The i&#39;th linear bucket covers the interval   [offset + (i-1) * width, offset + i * width) where i ranges from 1 to num_finite_buckets, inclusive.
 * @property {number} width The i&#39;th linear bucket covers the interval   [offset + (i-1) * width, offset + i * width) where i ranges from 1 to num_finite_buckets, inclusive. Must be strictly positive.
 */
/**
 * @typedef LogEntry
 * @memberOf! servicecontrol(v1)
 * @type object
 * @property {string} insertId A unique ID for the log entry used for deduplication. If omitted, the implementation will generate one based on operation_id.
 * @property {object} labels A set of user-defined (key, value) data that provides additional information about the log entry.
 * @property {string} name Required. The log to which this log entry belongs. Examples: `&quot;syslog&quot;`, `&quot;book_log&quot;`.
 * @property {object} protoPayload The log entry payload, represented as a protocol buffer that is expressed as a JSON object. The only accepted type currently is AuditLog.
 * @property {string} severity The severity of the log entry. The default value is `LogSeverity.DEFAULT`.
 * @property {object} structPayload The log entry payload, represented as a structure that is expressed as a JSON object.
 * @property {string} textPayload The log entry payload, represented as a Unicode string (UTF-8).
 * @property {string} timestamp The time the event described by the log entry occurred. If omitted, defaults to operation start time.
 */
/**
 * @typedef MetricValue
 * @memberOf! servicecontrol(v1)
 ...
 * @property {string} endTime The end of the time period over which this metric value&#39;s measurement applies.
 * @property {string} int64Value A signed 64-bit integer value.
 * @property {object} labels The labels describing the metric value. See comments on google.api.servicecontrol.v1.Operation.labels for the overriding relationship.
 * @property {servicecontrol(v1).Money} moneyValue A money value.
 * @property {string} startTime The start of the time period over which this metric value&#39;s measurement applies. The time period has different semantics for different metric types (cumulative, delta, and gauge). See the metric definition documentation in the service configuration for details.
 ...
 * @property {servicecontrol(v1).QuotaProperties} quotaProperties Represents the properties needed for quota check. Applicable only if this operation is for a quota check request. If this is not specified, no quota check will be performed.
 * @property {string} resourceContainer DO NOT USE. This field is deprecated, use &quot;resources&quot; field instead. The resource name of the parent of a resource in the resource hierarchy.  This can be in one of the following formats:     - “projects/&lt;project-id or project-number&gt;”     - “folders/&lt;folder-id&gt;”     - “organizations/&lt;organization-id&gt;”
 ...
 * @property {object} userLabels User defined labels for the resource that this operation is associated with. Only a combination of 1000 user labels per consumer project are allowed.
 */
/**
 * @typedef QuotaError
 * @memberOf! servicecontrol(v1)
 ...
 * @property {string} subject Subject to whom this error applies. See the specific enum for more details on this field. For example, &quot;clientip:&lt;ip address of client&gt;&quot; or &quot;project:&lt;Google developer project id&gt;&quot;.
 */
/**
 * @typedef QuotaInfo
 * @memberOf! servicecontrol(v1)
 * @type object
 * @property {string[]} limitExceeded Quota Metrics that have exceeded quota limits. For QuotaGroup-based quota, this is QuotaGroup.name For QuotaLimit-based quota, this is QuotaLimit.name See: google.api.Quota Deprecated: Use quota_metrics to get per quota group limit exceeded status.
 * @property {object} quotaConsumed Map of quota group name to the actual number of tokens consumed. If the quota check was not successful, then this will not be populated due to no quota consumption.  We are not merging this field with &#39;quota_metrics&#39; field because of the complexity of scaling in Chemist client code base. For simplicity, we will keep this field for Castor (that scales quota usage) and &#39;quota_metrics&#39; for SuperQuota (that doesn&#39;t scale quota usage).
 * @property {servicecontrol(v1).MetricValueSet[]} quotaMetrics Quota metrics to indicate the usage. Depending on the check request, one or more of the following metrics will be included:  1. For rate quota, per quota group or per quota metric incremental usage will be specified using the following delta metric:   &quot;serviceruntime.googleapis.com/api/consumer/quota_used_count&quot;  2. For allocation quota, per quota metric total usage will be specified using the following gauge metric:   &quot;serviceruntime.googleapis.com/allocation/consumer/quota_used_count&quot;  3. For both rate quota and allocation quota, the quota limit reached condition will be specified using the following boolean metric:   &quot;serviceruntime.googleapis.com/quota/exceeded&quot;
 */
/**
 * @typedef QuotaOperation
 * @memberOf! servicecontrol(v1)
 * @type object
 * @property {string} consumerId Identity of the consumer for whom this quota operation is being performed.  This can be in one of the following formats:   project:&lt;project_id&gt;,   project_number:&lt;project_number&gt;,   api_key:&lt;api_key&gt;.
 * @property {object} labels Labels describing the operation.
 * @property {string} methodName Fully qualified name of the API method for which this quota operation is requested. This name is used for matching quota rules or metric rules and billing status rules defined in service configuration.  This field should not be set if any of the following is true: (1) the quota operation is performed on non-API resources. (2) quota_metrics is set because the caller is doing quota override.  Example of an RPC method name:     google.example.library.v1.LibraryService.CreateShelf
 ...
 * @memberOf! servicecontrol(v1)
 * @type object
 * @property {string} operationId The same operation_id value used in the StartReconciliationRequest. Used for logging and diagnostics purposes.
 * @property {servicecontrol(v1).MetricValueSet[]} quotaMetrics Metric values as tracked by One Platform before the start of reconciliation. The following metrics will be included:  1. Per quota metric total usage will be specified using the following gauge metric:   &quot;serviceruntime.googleapis.com/allocation/consumer/quota_used_count&quot;  2. Value for each quota limit associated with the metrics will be specified using the following gauge metric:   &quot;serviceruntime.googleapis.com/quota/limit&quot;
