        ...
        devices:
            {
              ...
              list(params, options, callback) {
                ...
                return createAPIRequest(parameters, callback);
                  ...
                  * @desc Modifies the configuration for the device, which is
                  * eventually sent from the Cloud IoT servers. Returns the
                  * modified configuration version and its meta-data.
                  * @alias
                  * cloudiot.projects.locations.registries.devices.modifyCloudToDeviceConfig
                  * @memberOf! cloudiot(v1beta1)
                  *
              ...
              patch(params, options, callback) {
                ...
                return createAPIRequest(parameters, callback);
                  ...
                  * @return {object} Request object
                  */
              publishEvent(params, options, callback) {
                if (typeof options === 'function') {
                  callback = options;
                  options = {};
                }
 ...
 * @memberOf! cloudiot(v1beta1)
 * @type object
 * @property {cloudiot(v1beta1).AuditLogConfig[]} auditLogConfigs The configuration for logging of each type of permission. Next ID: 4
 ...
 * @memberOf! cloudiot(v1beta1)
 * @type object
 * @property {cloudiot(v1beta1).Expr} condition The condition that is associated with this binding. NOTE: an unsatisfied condition will not allow user access via current binding. Different bindings, including their conditions, are examined independently. This field is GOOGLE_INTERNAL.
 ...
 * @memberOf! cloudiot(v1beta1)
 * @type object
 * @property {cloudiot(v1beta1).DeviceConfig} config The most recent device configuration, which is eventually sent from the Cloud IoT Core service to the device. If not present on creation, the configuration will be initialized with an empty payload and version value of `1`. To update this field after creation, use the `DeviceManager.ModifyCloudToDeviceConfig` method.
 * @property {cloudiot(v1beta1).DeviceCredential[]} credentials The credentials used to authenticate this device. To allow credential rotation without interruption, multiple device credentials can be bound to this device. No more than 3 credentials can be bound to a single device at a time.
 * @property {string} enabledState If a device is disabled, communication from it will be blocked. Can be used to temporarily prevent the device from connecting if, for example, the sensor is generating bad data and needs maintenance.
 * @property {string} id The user-defined device identifier. The device ID must be unique within a device registry.
 * @property {string} lastConfigAckTime [Output only] The last time a cloud-to-device config version acknowledgment was received from the device.
 * @property {cloudiot(v1beta1).Status} lastErrorStatus [Output only] The error message of the last error, e.g., failed to publish to Cloud Pub/Sub. &#39;last_error_time&#39; is the timestamp of this field. If no errors are present, this will have an empty message (whose status code is 0 == OK), otherwise this field is expected to have a not-OK status code.
 * @property {string} lastErrorTime [Output only] The last time an error happened, e.g., failed to publish to Cloud Pub/Sub. This field is the timestamp of &#39;last_error_status&#39;.
 * @property {string} lastEventTime [Output only] The last time an event was received. Timestamps are periodically collected and written to storage; they may be stale by a few minutes.
 * @property {string} lastHeartbeatTime [Output only] The last time a heartbeat was received. Timestamps are periodically collected and written to storage; they may be stale by a few minutes.
 ...
 * @memberOf! cloudiot(v1beta1)
 * @type object
 * @property {string} cloudUpdateTime [Output only] The time when this version state was updated in the Cloud IoT Core service. This timestamp is set by the server.
 * @property {cloudiot(v1beta1).DeviceConfigData} data The device configuration data.
 * @property {string} deviceAckTime [Output only] The time when the Cloud IoT Core server received the acknowledgment from the device, indicating that the device has received this configuration version. If this field is not present, the device has not yet acknowledged that it received this version. Note that when sending the config to the device, there may have been many config versions on the Cloud IoT Core service while the device was disconnected; and on connection, only the latest version is sent to the device. Some of the versions may never be sent to the device, and therefore are never acknowledged. This timestamp is set by the Cloud IoT Core service.
 * @property {string} version [Output only] The version of this update. The version number is assigned by the server, and is always greater than zero after device creation. The version must be zero on the `CreateDevice` request if a `config` is specified; the response of `CreateDevice` will always have a value of one.
 */
/**
 ...
 * @property {cloudiot(v1beta1).MqttConfig} mqttConfig The configuration of the MQTT broker associated with this device registry, including enablement, payload data format type, etc.
 ...
 * @property {string} subFolder Optional subfolder for the telemetry event. This can be used to classify types of events, and is included in the Pub/Sub message attributes.
 */
/**
 ...
 * @memberOf! cloudiot(v1beta1)
 * @type object
 * @property {cloudiot(v1beta1).DeviceConfig[]} deviceConfigs The device configuration for the last few versions. Versions are listed in decreasing order, starting from the most recent one.
 */
/**
 ...
 * @memberOf! cloudiot(v1beta1)
 * @type object
 * @property {string} mqttConfigState If enabled, allows connections using the MQTT protocol. Otherwise any MQTT connection to this registry will fail.
 */
/**
 ...
 * @property {cloudiot(v1beta1).Binding[]} bindings Associates a list of `members` to a `role`. `bindings` with no members will result in an error.
 ...
 * @memberOf! cloudiot(v1beta1)
 * @type object
 * @property {cloudiot(v1beta1).Policy} policy REQUIRED: The complete policy to be applied to the `resource`. The size of the policy is limited to a few 10s of KB. An empty policy is a valid policy but certain Cloud Platform services (such as Projects) might reject them.
