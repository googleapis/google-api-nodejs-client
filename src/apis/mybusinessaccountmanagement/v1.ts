  ...
  export class Resource$Accounts$Invitations {
    ...
    list(
      ...
      const parameters = {
        options: Object.assign(
          {
            url: (rootUrl + '/v1/{+parent}/invitations')
              .replace(/([^:]\/)\/+/g, '$1')
              .replace(/\{([^+:][^}]*)\}/g, '{+$1}'),
            method: 'GET',
            apiVersion: '',
