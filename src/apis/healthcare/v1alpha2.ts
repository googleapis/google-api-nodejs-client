...
export namespace healthcare_v1alpha2 {
  ...
  export class Resource$Projects$Locations$Datasets$Fhirstores$Fhir {
     ...
     * @param {string=} params.pageToken Used to retrieve the next or previous page of results when using pagination. Value should be set to the value of page_token set in next or previous page links' url. Next and previous page are returned in the response bundle's links field, where `link.relation` is "previous" or "next".  Omit `page_token` if no previous request has been made.
