...
export namespace firestore_v1beta1 {
  ...
  export interface Schema$Aggregation {
    /**
     * Required. The name of the field to store the result of the aggregation into. Requires: * Must be present. * Must be unique across all aggregation aliases. * Conform to existing document field name limitations.
     */
    alias?: string | null;
    /**
     * Count aggregator.
  ...
  export interface Schema$Count {
    /**
     * Optional. Optional constraint on the maximum number of documents to count. This provides a way to set an upper bound on the number of documents to scan, limiting latency and cost. High-Level Example: ``` SELECT COUNT_UP_TO(1000) FROM ( SELECT * FROM k ); ``` Requires: * Must be greater than zero when present.
     */
    upTo?: number | null;
  }
  /**
  ...
  export interface Schema$RunAggregationQueryResponse {
    ...
    readTime?: string | null;
    /**
     * A single aggregation result. Not present when reporting partial progress or when the query produced zero results.
     */
