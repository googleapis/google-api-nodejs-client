...
BatchTransporter.prototype.wrapCallback_ = function(opt_callback) {
  return function(err, body, res) {
    ...
    //If the transporter passed in an error, or the body doesn't exist, just call the callback with existing parameters.
    if (err || !body) {
    ...
    //Content type header also contains the boundary name, so extract it out of the string with regex
