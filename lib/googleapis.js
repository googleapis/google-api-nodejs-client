...
GoogleApis.prototype.load = function(api, opt_callback) {
  ...
  if (data) {
    ...
    var localDiscoveryFile = api.opts && api.opts.localDiscoveryFile;
    if (localDiscoveryFile) {
      fs.readFile(localDiscoveryFile, function(err, data) {
        generateClient(err, data && JSON.parse(data));
      });
    } else {
