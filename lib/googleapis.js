...
GoogleApisClient.prototype.add = function(name, client) {
	...
	if ( version[0] != 'v' && version[1].search(/[0-9]/) == -1 ) {
		if ( !(name in this) ) {
			this[name] = {};
		}
		this[name][client.getCanonicalName()] = client.withAuthClient(this.authClient);
...
GoogleApis.prototype.discover = function(name, version, opt_opts) {
  var alreadyDiscovered = this.toBeDiscovered.some(function(discovery) {
    return (discovery.name === name && discovery.version === version);
  });
