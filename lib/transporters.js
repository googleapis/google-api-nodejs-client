...
DefaultTransporter.prototype.wrapCallback_ = function(opt_callback) {
  return function(err, res, body) {
    ...
    // if multipart/mixed, parse the stream for each component
    // TODO: dont buffer multipart responses,
    // pipe request output stream to parser.
    if (res.headers && res.headers['content-type']
          && res.headers['content-type'].indexOf('multipart/mixed') >= 0) {
      // todo: parse the body
      return opt_callback && opt_callback(err, res, body);
    }
