 ...
 * @return {object} Credentials instance.
 */
...
GoogleCredentials.prototype.fromStream = function(stream, opt_callback) {
  ...
  if (!stream) {
    if (opt_callback) {
      opt_callback(new Error('Must pass in a stream containing the service account auth settings.'));
    }
  } else {
    var parser = JSONStream.parse("*");

    parser.on('data', function (data) {
      var creds = that.fromJSON(data);

      if (opt_callback) {
        opt_callback(null, creds);
      }
    })
