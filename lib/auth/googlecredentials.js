 ...
 * @private
 */
GoogleCredentials.prototype._is_gce = false;

/**
 * Caches a value indicating whether we've checked whether the auth layer is running on Google Compute Engine.
 * @private
 */
GoogleCredentials.prototype._checked_is_gce = false;
  ...
  if (that._cachedCredential) {
    ...
    if (!that._tryGetApplicationCredentialsFromEnvironmentVariable(my_callback)) {
      ...
      if (!that._tryGetApplicationCredentialsFromWellKnownFile(my_callback)) {
        ...
        // Next, determine if we're running on GCE.
        that._checkIsGCE(function(err) {
          if (err) {
            callback(opt_callback, err);
          }

          // If we're running on GCE, get the credentials from GCE.
          if (that._is_gce) {
            this._getApplicationCredentialsFromGCE(my_callback);
          } else {
 ...
 * @api private
 */
GoogleCredentials.prototype._checkIsGCE = function(callback) {
  var that = this;

  if (that._checked_is_gce) {
    callback();
  } else {
    if (!that.transporter) {
    ...
    that.transporter.request({
      ...
      if (!err && res && res.headers) {
        that._is_gce = res.headers['Metadata-Flavor'] == 'Google';
      }

      that._checked_is_gce = true;
      callback();
    });
  }
}
...
/**
 * Attempts to load credentials from Google Compute Engine.
 * @param {function=} opt_callback Optional callback.
 * @return {boolean} Returns true if the callback has been executed; false otherwise.
 * @api private
 */
GoogleCredentials.prototype._getApplicationCredentialsFromGCE = function(opt_callback) {
  var that = this;

  if (!that._is_gce) {
    throw new Error('_getApplicationCredentialsFromGCE should not be called unless running on GCE');
  }
}
