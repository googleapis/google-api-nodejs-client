 ...
 * @param {boolean=} gce indicates whether the auth layer is running on a Google Compute Engine instance.
 * @constructor
 */
...
JWT.prototype.refreshToken_ = function(ignored_, opt_callback) {
  ...
  if (that.gce) {
    // When running on GCE, use our own token request code.
    this.getTokenFromGCE(opt_callback);
  } else {
...
/**
 * Refreshes the access token using the GCR metadata endpoint.
 * @param {function=} opt_callback Optional callback.
 * @private
 */
JWT.prototype._getTokenFromGCE = function(opt_callback) {
