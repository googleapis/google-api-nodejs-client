...
/**
 * Helper for formatting the incoming scopes.
 */
function convertScopes(scopes) {
  return scopes instanceof Array ? scopes.join(' ') : scopes;
}
...
JWT.prototype.authorize = function(opt_callback) {
  ...
  that.gapi = new that.GAPI({
    iss: that.email,
    sub: that.subject,
    scope: convertScopes(that.scopes),
 ...
 * @param {(string|array)=} scopes list of requested scopes or a single scope.
 */
JWT.prototype.setScopes = function(scopes) {
  this.scopes = convertScopes(scopes);
}
