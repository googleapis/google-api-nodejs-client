...
/**
 * Google Compute Engine refresh token placeholder.
 * @private
 */
Compute.TOKEN_PLACEHOLDER = 'compute-placeholder';
...
function Compute() {
  ...
  this.credentials = { refresh_token: Compute.TOKEN_PLACEHOLDER };
}
...
Compute.prototype.authorize = function(opt_callback) {
  ...
  that.refreshToken_(null, function(err, result) {
    if (!err) {
      ...
      that.credentials.refresh_token = Compute.TOKEN_PLACEHOLDER;
    }
    if (opt_callback) {
