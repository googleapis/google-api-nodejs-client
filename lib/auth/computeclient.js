...
function Compute() {
  ...
  this._postRequest = this._postComputeRequest;
}
...
Compute.prototype.createScopedRequired = function() {
  ...
  // and cannot be changed. For this reason, always return false here.
  return false;
}
 ...
 * @return {object} The cloned instance.
 */
Compute.prototype.createScoped = function(scopes) {
  // On compute engine, scopes are specified at the compute instance's creation time,
  // and cannot be changed. This method is never required. Throw an exception.
  throw new Error('The createScoped method is not required when running on Google Compute Engine. ' +
    'Check the value of the createdScopedRequired method before calling this function.');
}
/**
 ...
 * @private
 */
Compute.prototype._postComputeRequest = function(err, result, callback) {
  ...
  if (err && err.code) {
    ...
    if (err.code == 403) {
      helpfulMessage = 'A Forbidden error code was returned. This may be because the Compute Engine instance does not ' +
        'have the correct permission scopes specified.';
    } else if (err.code == 404) {
      helpfulMessage = 'A Not Found error code was returned. This may be because the Compute Engine instance does not ' +
        'have any permission scopes specified.';
    }
