...
Compute.prototype.createScoped = function(scopes) {
  // On compute engine, scopes are specified at the compute instance's creation time,
  // and cannot be changed. This method is never required.
  return this;
}
 ...
 * @private
 */
Compute.prototype._postComputeRequest = function(err, result, response, callback) {
  var statusCode = null;
  ...
  if (statusCode) {
    ...
    if (helpfulMessage) {
      ...
      // Since we re-created the Error instance, repopulate the status code.
      if (statusCode) {
        err.code = statusCode;
      }
    }
