...
OAuth2Client.prototype.request = function(opts, callback) {
  ...
  var my_callback = function(e, r) {
    that._postRequest(e, r, callback);
  };

  if (!credentials.access_token && !credentials.refresh_token) {
    my_callback(new Error('No access or refresh token is set.'), null);
    return;
  }
  ...
  if (shouldRefresh && credentials.refresh_token) {
    this.refreshAccessToken(function(err, tokens) {
      if (err) {
        my_callback(err, null);
      } else if (!tokens || (tokens && !tokens.access_token)) {
        my_callback(new Error('Could not refresh access token.'), null);
      } else {
        return that._makeRequest(opts, my_callback);
      }
    });
  } else {
    return this._makeRequest(opts, my_callback);
  }
};
 ...
 * @private
 */
OAuth2Client.prototype._postRequest = function(err, result, callback) {
  callback(err, result);
}
