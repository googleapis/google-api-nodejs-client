...
OAuth2Client.prototype.verifyIdToken =
  ...
  var that = this;
  this.getFederatedSignonCerts(function(certs) {
    var login = that.verifySignedJwtWithCerts(idToken, certs, audience);
    var err = null;
    if (!login) {
      err = 'Unable to verify the ID Token';
    }
    callback(err, login);
  });
};
...
OAuth2Client.prototype.getFederatedSignonCerts = function(callback) {
  ...
  this.transporter.request({
    method: 'GET',
    uri: OAuth2Client.GOOGLE_OAUTH2_FEDERATED_SIGNON_CERTS_URL,
    json: true
  }, function(err, body, res) {
    if (err) {
      throw new Error('Failed to retrieve verification certificates: ' + err);
    }
...
OAuth2Client.prototype.verifySignedJwtWithCerts =
  function(jwt, certs, requiredAudience) {
  ...
  var envelope = JSON.parse(this.decodeBase64(segments[0]));
  if (!envelope) {
    throw new Error('Can\'t parse token envelope: ' + segments[0]);
  }

  var payload = JSON.parse(this.decodeBase64(segments[1]));
  if (!payload) {
  ...
  if (exp >= now + this.MAX_TOKEN_LIFETIME_SECS) {
    throw new Error('Expiration time too far in future: ' +
      JSON.stringify(payload));
  }

  var earliest = iat - this.CLOCK_SKEW_SECS;
  var latest = exp + this.CLOCK_SKEW_SECS;
