...
function Proximitybeacon(options) {
  ...
  self.beacons = {
    ...
    , /**
       ...
       * will no longer be returned from `beaconinfo.getforobserved`. This
       * operation is permanent -- you will not be able to re-register a beacon
    ...
    , /**
       ...
       * the beacon as well as any attachments on the beacon (including those
    ...
    attachments: {
      ...
      , /**
         ...
         * actual attachment data itself.</li> </ul> The namespaced type
         * consists of two parts, the namespace and the type. The namespace must
         * be one of the values returned by the `namespaces` endpoint, while the
         * type can be a string of any characters except for the forward slash
         * (`/`) up to 100 characters in length.  Attachment data can be up to
         * 1024 bytes long.  Authenticate using an [OAuth access
      ...
      , /**
         ...
         * request. You must either use `x/x`, to return all attachments, or the
 ...
 * @memberOf! proximitybeacon(v1beta1)
 * @type object
 * @property {string} id The actual beacon identifier, as broadcast by the beacon hardware. Must be
[base64](http://tools.ietf.org/html/rfc4648#section-4) encoded in HTTP
requests, and will be so encoded (with padding) in responses. The base64
encoding should be of the binary byte-stream and not any textual (such as
hex) representation thereof.
Required.
 * @property {string} type Specifies the identifier type.
Required.
 */
/**
 ...
 * @property {number} maxDistanceMeters The distance away from the beacon at which this attachment should be
...
this distance away from the beacon.
...
version.
Optional.
 * @property {string} provisioningKey Some beacons may require a user to provide an authorization key before
...
Namespace provides type separation between clients. Type describes the type of
...
be the beacon&#39;s current EID, or the beacon&#39;s &quot;stable&quot;
Eddystone-UID.
 ...
 * @memberOf! proximitybeacon(v1beta1)
 * @type object
 * @property {proximitybeacon(v1beta1).BeaconInfo[]} beacons Public information about beacons.
 ...
 * @property {string} nextPageToken Token that can be used for pagination. Returned only if the
 ...
 * @memberOf! proximitybeacon(v1beta1)
 * @type object
 * @property {string} namespaceName Resource name of this namespace. Namespaces names have the format:
&lt;code&gt;namespaces/&lt;var&gt;namespace&lt;/var&gt;&lt;/code&gt;.
 * @property {string} servingVisibility Specifies what clients may receive attachments under this namespace
