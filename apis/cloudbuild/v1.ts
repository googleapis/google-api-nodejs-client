...
function Cloudbuild(options) {
  ...
  self.operations = {
    ...
    , /**
       ...
       * use different resource name schemes, such as `users/x/operations`. To
  ...
  self.projects = {
    builds: {
      ...
      , /**
         ...
         * long-running Operation returned by this method will include the ID of
      ...
      , /**
         ...
         * Build that is returned includes its status (e.g., success or failure,
      ...
      , /**
         ...
         * @desc Lists previously requested builds.  Previously requested builds
      ...
      , /**
         ...
         * @desc Creates a new build based on the given build.  This API creates
         * a new build using the original build request,  which may or may not
         * result in an identical build.  For triggered builds:  * Triggered
         * builds resolve to a precise revision, so a retry of a triggered build
         * will result in a build that uses the same revision.  For
         * non-triggered builds that specify RepoSource:  * If the original
    ...
    triggers: {
      ...
      , /**
         ...
         * @desc Deletes an BuildTrigger by its project ID and trigger ID.  This
         * API is experimental.
      ...
      , /**
         ...
         * @desc Updates an BuildTrigger by its project ID and trigger ID.  This
         * API is experimental.
 ...
 * @memberOf! cloudbuild(v1)
 * @type object
 * @property {string} diskSizeGb Requested disk size for the VM that runs the build. Note that this is *NOT*
&quot;disk free&quot;; some of the space will be used by the operating system
and build utilities. Also note that this is the minimum disk size that will be
...
If the image is already available in the host&#39;s Docker daemon&#39;s cache,
it will be run directly. If not, the host will attempt to pull the image first,
...
images, like &quot;ubuntu&quot;, &quot;debian&quot;, but they will be refreshed
