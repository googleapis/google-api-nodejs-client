    ...
    instances:
        {
           ...
           * `CreateInstance` returns `ALREADY_EXISTS`.  Immediately upon
           * completion of this request:    * The instance is readable via the
           * API, with all requested attributes     but no allocated resources.
           * Its state is `CREATING`.  Until completion of the returned
           * operation:    * Cancelling the operation renders the instance
           * immediately unreadable     via the API.   * The instance can be
           * deleted.   * All other attempts to modify the instance are
           * rejected.  Upon completion of the returned operation:    * Billing
           * for all successfully-allocated resources begins (some types     may
           * have lower than the requested levels).   * Databases can be created
           * in the instance.   * The instance's allocated resource levels are
          ...
          , /**
             ...
             * @desc Deletes an instance.  Immediately upon completion of the
             * request:    * Billing ceases for all of the instance's reserved
             * resources.  Soon afterward:    * The instance and *all of its
             * databases* immediately and     irrevocably disappear from the
          ...
          , /**
             ...
             * @desc Gets the access control policy for an instance resource.
             * Returns an empty policy if an instance exists but does not have a
             * policy set.  Authorization requires
          ...
          , /**
             ...
             * @desc Updates an instance, and begins allocating or releasing
             ...
             * named instance does not exist, returns `NOT_FOUND`.  Immediately
             * upon completion of this request:    * For resource types for
             * which a decrease in the instance's allocation     has been
             * requested, billing is based on the newly-requested level.  Until
             * completion of the returned operation:    * Cancelling the
             * operation sets its metadata's     cancel_time, and begins
             ...
             * operation:    * Billing begins for all successfully-allocated
             * resources (some types     may have lower than the requested
             * levels).   * All newly-reserved resources are available for
             * serving the instance's     tables.   * The instance's new
             * resource levels are readable via the API.  The returned
          ...
          , /**
             ...
             * @desc Sets the access control policy on an instance resource.
          ...
          , /**
             ...
             * @desc Returns permissions that the caller has on the specified
             * instance resource.  Attempting this RPC on a non-existent Cloud
             * Spanner instance resource will result in a NOT_FOUND error if the
             * user has `spanner.instances.list` permission on the containing
             * Google Cloud Project. Otherwise returns an empty set of
             * permissions.
          ...
          databases:
              {
                 ...
                 * prepare it for serving. The returned long-running operation
                 * will have a name of the format
                 * `<database_name>/operations/<operation_id>` and can be used
                 * to track preparation of the database. The metadata field type
                 * is CreateDatabaseMetadata. The response field type is
                ...
                , /**
                   ...
                   * @desc Returns the schema of a Cloud Spanner database as a
                   * list of formatted DDL statements. This method does not show
                   * pending schema updates, those may be queried using the
                   * Operations API.
                ...
                , /**
                   ...
                   * resource. Returns an empty policy if a database exists but
                   * does not have a policy set.  Authorization requires
                   * `spanner.databases.getIamPolicy` permission on resource.
                ...
                , /**
                   ...
                   * requires `spanner.databases.setIamPolicy` permission on
                   * resource.
                ...
                , /**
                   ...
                   * NOT_FOUND error if the user has `spanner.databases.list`
                   * permission on the containing Cloud Spanner instance.
                ...
                , /**
                   ...
                   * format `<database_name>/operations/<operation_id>` and can
                   * be used to track execution of the schema change(s). The
                ...
                operations: {
                   ...
                   * completed despite cancellation. On successful cancellation,
                   * the operation is not deleted; instead, it becomes an
                  ...
                  , /**
                     ...
                     * operation result. It does not cancel the operation. If
                  ...
                  , /**
                     ...
                     * the request. If the server doesn't support this method,
                     * it returns `UNIMPLEMENTED`.  NOTE: the `name` binding
                     * allows API services to override the binding to use
                     * different resource name schemes, such as
                ...
                sessions: {
                  ...
                  , /**
                     ...
                     * @desc Commits a transaction. The request includes the
                     * mutations to be applied to rows in the database.
                     * `Commit` might return an `ABORTED` error. This can occur
                     * at any time; commonly, the cause is conflicts with
                     * concurrent transactions. However, it can also happen for
                     * a variety of other reasons. If `Commit` returns
                     * `ABORTED`, the caller should re-attempt the transaction
                  ...
                  , /**
                     ...
                     * @desc Creates a new session. A session can be used to
                     * perform transactions that read and/or modify data in a
                     * Cloud Spanner database. Sessions are meant to be reused
                     * for many consecutive transactions.  Sessions can only
                     * execute one transaction at a time. To execute multiple
                     * concurrent read-write/write-only transactions, create
                     * multiple sessions. Note that standalone reads and queries
                     * use a transaction internally, and count toward the one
                     * transaction limit.  Cloud Spanner limits the number of
                     * sessions that can exist at any given time; thus, it is a
                     * good idea to delete idle and/or unneeded sessions. Aside
                     * from explicit deletes, Cloud Spanner can delete sessions
                     * for which no operations are sent for more than an hour.
                     * If a session is deleted, requests to it return
                     * `NOT_FOUND`.  Idle sessions can be kept alive by sending
                     * a trivial SQL query periodically, e.g., `"SELECT 1"`.
                     * @alias
                     * spanner.projects.instances.databases.sessions.create
                     * @memberOf! spanner(v1)
                     *
                  ...
                  , /**
                     ...
                     * result set larger than 10 MiB; if the query yields more
                     * data than that, the query fails with a
                     * `FAILED_PRECONDITION` error.  Queries inside read-write
                     * transactions might return `ABORTED`. If this occurs, the
                     * application should restart the transaction from the
                     * beginning. See Transaction for more details.  Larger
                     * result sets can be fetched in streaming fashion by
                  ...
                  , /**
                     ...
                     * @desc Like ExecuteSql, except returns the result set as a
                     * stream. Unlike ExecuteSql, there is no limit on the size
                     * of the returned result set. However, no individual row in
                     * the result set can exceed 100 MiB, and no column value
                     * can exceed 10 MiB.
                     * @alias
                     * spanner.projects.instances.databases.sessions.executeStreamingSql
                     * @memberOf! spanner(v1)
                  ...
                  , /**
                     ...
                     * @desc Gets a session. Returns `NOT_FOUND` if the session
                     * does not exist. This is mainly useful for determining
                  ...
                  , /**
                     ...
                     * @desc Reads rows from the database using key lookups and
                     * scans, as a simple key/value style alternative to
                     * ExecuteSql.  This method cannot be used to return a
                     * result set larger than 10 MiB; if the read matches more
                     * data than that, the read fails with a
                     * `FAILED_PRECONDITION` error.  Reads inside read-write
                     * transactions might return `ABORTED`. If this occurs, the
                     * application should restart the transaction from the
                     * beginning. See Transaction for more details.  Larger
                     * result sets can be yielded in streaming fashion by
                  ...
                  , /**
                     ...
                     * @desc Rolls back a transaction, releasing any locks it
                     * holds. It is a good idea to call this for any transaction
                     * that includes one or more Read or ExecuteSql requests and
                     * ultimately decides not to commit.  `Rollback` returns
                     * `OK` if it successfully aborts the transaction, the
                     * transaction was already aborted, or the transaction is
                     * not found. `Rollback` never returns `ABORTED`.
                     * @alias
                     * spanner.projects.instances.databases.sessions.rollback
                     * @memberOf! spanner(v1)
                     *
                  ...
                  , /**
                     ...
                     * stream. Unlike Read, there is no limit on the size of the
                     * returned result set. However, no individual row in the
                     * result set can exceed 100 MiB, and no column value can
                     * exceed 10 MiB.
                     * @alias
                     * spanner.projects.instances.databases.sessions.streamingRead
                     * @memberOf! spanner(v1)
          ...
          operations: {
            ...
            , /**
               ...
               * must ensure the name binding is the parent resource, without
...
[][google.spanner.admin.instance.v1.Instance] from being erased accidentally by
...
[][google.spanner.admin.instance.v1.UpdateInstanceRequest.field_mask] need be
included.
 */
/**
 * @typedef Write
