  ...
  self.projects = {
    locations: {
      ...
      , /**
         ...
         * applicable. name: 'projects/my-project',  // TODO: Update placeholder
         * value.
         *
         *     auth: authClient,
         *   };
      ...
      queues: {
         ...
         * @desc Creates a queue.  WARNING: Using this method may have
         ...
         * Management and queue.yaml](/cloud-tasks/docs/queue-yaml) carefully
        ...
        , /**
           ...
           * queue.yaml](/cloud-tasks/docs/queue-yaml) carefully before using
           * this method.
           * @example
           * // BEFORE RUNNING:
           * // ---------------
        ...
        , /**
           ...
           * not exist and updates the queue if it does exist.  WARNING: Using
           * this method may have unintended side effects if you are using an
           * App Engine `queue.yaml` or `queue.xml` file to manage your queues.
        ...
        , /**
           ...
           * stop executing the tasks in the queue until it is resumed via
           * CloudTasks.ResumeQueue. Tasks can still be added when the queue is
           * paused. The state of the queue is stored in Queue.queue_state; if
           * paused it will be set to Queue.QueueState.PAUSED.
           * @example
           * // BEFORE RUNNING:
           * // ---------------
           * // 1. If not already done, enable the Cloud Tasks API
        ...
        , /**
           ...
           * been Queue.QueueState.PAUSED or Queue.QueueState.DISABLED. The
           * state of a queue is stored in Queue.queue_state; after calling this
           * method it will be set to Queue.QueueState.RUNNING.  WARNING:
           * Resuming many high-QPS queues at the same time can lead to target
           * overloading. If you are resuming high-QPS queues, follow the
        ...
        , /**
           ...
           * permissions, not a google.rpc.Code.NOT_FOUND error.  Note: This
        ...
        tasks:
            {
               ...
               * @desc Acknowledges a pull task.  The lease holder, that is, the
               * entity that received this task in a PullTasksResponse, must
               * call this method to indicate that the work associated with the
               * task has finished.  The lease holder must acknowledge a task
               * within the PullTasksRequest.lease_duration or the lease will
               * expire and the task will become ready to be returned in a
               * different PullTasksResponse. After the task is acknowledged, it
               * will not be returned by a later CloudTasks.PullTasks,
               * CloudTasks.GetTask, or CloudTasks.ListTasks.  To acknowledge
              ...
              , /**
                 ...
                 * @desc Cancel a pull task's lease.  The lease holder can use
                 * this method to cancel a task's lease by setting
                 * Task.schedule_time to now. This will make the task available
                 * to be leased to the next caller of CloudTasks.PullTasks.
                 * @example
                 * // BEFORE RUNNING:
                 * // ---------------
                 * // 1. If not already done, enable the Cloud Tasks API
              ...
              , /**
                 ...
                 * @desc Deletes a task.  A task can be deleted if it is
                 * scheduled or dispatched. A task cannot be deleted if it has
              ...
              , /**
                 ...
                 * @desc Lists the tasks in a queue.  By default response_view
                 * is Task.View.BASIC; not all information is retrieved by
                 * default due to performance considerations;
                 * ListTasksRequest.response_view controls the subset of
                 ...
                 *       // TODO: Change code below to process each resource in
                 * `tasksPage`: console.log(JSON.stringify(tasksPage[i], null,
                 * 2));
                 *     }
                 *
                 *     if (response.nextPageToken) {
              ...
              , /**
                 ...
                 * @desc Pulls tasks from a pull queue and acquires a lease on
                 * them for a specified PullTasksRequest.lease_duration.  This
                 * method is invoked by the lease holder to obtain the lease.
                 * The lease holder must acknowledge the task via
                 * CloudTasks.AcknowledgeTask after they have performed the work
                 * associated with the task.  The payload is intended to store
                 * data that the lease holder needs to perform the work
                 * associated with the task. To return the payloads in the
                 * PullTasksResponse, set PullTasksRequest.response_view to
                 * Task.View.FULL.  A maximum of 10 qps of CloudTasks.PullTasks
                 * requests are allowed per queue.
                 * google.rpc.Code.RESOURCE_EXHAUSTED is returned when this
                 * limit is exceeded. google.rpc.Code.RESOURCE_EXHAUSTED is also
                 * returned when RateLimits.max_tasks_dispatched_per_second is
                 * exceeded.
                 * @example
                 * // BEFORE RUNNING:
                 * // ---------------
                 ...
                 *   };
                 *
                 *   cloudTasks.projects.locations.queues.tasks.pull(request,
                 * function(err, response) { if (err) { console.error(err);
                 *       return;
                 *     }
                 *
              ...
              pull: function(params, options, callback) {
                ...
                const parameters = {
                  options: Object.assign(
                      {
                        url: (rootUrl + '/v2beta2/{name}/tasks:pull')
                                 .replace(/([^:]\/)\/+/g, '$1'),
                        method: 'POST'
                      },
                      options),
              ...
              , /**
                 ...
                 * @desc Renew the current lease of a pull task.  The lease
                 * holder can use this method to extend the lease by a new
              ...
              , /**
                 ...
                 * used for manual debugging. For example, CloudTasks.RunTask
                 * can be used to retry a failed task after a fix has been made
                 * or to manually force a task to be dispatched now.  When this
                 * method is called, Cloud Tasks will dispatch the task to its
                 * target, even if the queue is Queue.QueueState.PAUSED.  The
                 * dispatched task is returned. That is, the task that is
                 * returned contains the Task.task_status after the task is
                 ...
                 * in the queue and task's RetryConfig.  CloudTasks.RunTask
                 * returns google.rpc.Code.NOT_FOUND when it is called on a task
                 * that has already succeeded or permanently failed.
                 * google.rpc.Code.FAILED_PRECONDITION is returned when
...
explictly setting `Content-Type` to a particular media type when the [task is
...
handle HTTP requests with this http_method, otherwise the task attempt will fail
...
URL. It can contain a path and query string arguments. If the relative URL is
empty, then the root path &quot;/&quot; will be used. No spaces are allowed, and
