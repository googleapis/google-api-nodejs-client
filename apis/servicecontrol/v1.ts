...
function Servicecontrol(options) {
  ...
  self.services = {
    ...
    , /**
       ...
       * operation is executed.  If feasible, the client should cache the check
       * results and reuse them for 60 seconds. In case of server errors, the
       * client can rely on the cached results for longer time.  NOTE: the
    ...
    , /**
       ...
       * these errors to prohibit any hard dependency on the quota
       * functionality.
    ...
    , /**
       ...
       * feasible, the client should aggregate reporting data for up to 5
       * seconds to reduce API traffic. Limiting aggregation to 5 seconds is to
       * reduce data loss during client crashes. Clients should carefully choose
       * the aggregation time window to avoid data loss risk more than 0.01% for
    ...
    , /**
       ...
       * Services can invoke StartReconciliation and EndReconciliation to
       * correct this usage drift, as described below: 1. Service sends
       ...
       * EndReconciliation is    called. 3. At or after the time specified in
       * the StartReconciliation, service    sends EndReconciliation with the
       * usage that needs to be reconciled to. 4. One Platform adjusts its own
       * record of usage for that metric to the    value specified in
       * EndReconciliation by taking in to account any    allocation or release
       * between StartReconciliation and EndReconciliation.  Signals the quota
       * controller that the service wants to perform a usage reconciliation as
       * specified in the request.  This method requires the
       * `servicemanagement.services.quota` permission on the specified service.
       * For more information, see [Google Cloud
       * IAM](https://cloud.google.com/iam).
