 ...
 * @property {string} planName The planName property is required. This is the name of the subscription&#39;s payment plan. For more information about the Google payment plans, see API concepts.
 ...
 * @property {integer} maximumNumberOfSeats The maximumNumberOfSeats property is the maximum number of licenses that the customer can purchase. This property applies to plans other than the annual commitment plan. How a user&#39;s licenses are managed depends on the subscription&#39;s payment plan:
...
API&#39;s response.
- flexible plan — For this plan, a reseller is invoiced on the actual number of
users which is capped by the maximumNumberOfSeats. This is the maximum number of
user licenses a customer has for user license provisioning. This quantity can be
increased up to the maximum limit defined in the reseller&#39;s contract. And
...
to maximum 10 seats.
 * @property {integer} numberOfSeats The numberOfSeats property holds the customer&#39;s number of user licenses. How a user&#39;s licenses are managed depends on the subscription&#39;s plan:
...
provisioned users.
- flexible plan — For this plan, a reseller is invoiced on the actual number of
users which is capped by the maximumNumberOfSeats. The numberOfSeats property is
not used in the request or response for flexible plan customers.
 ...
 * @property {string[]} suspensionReasons Read-only field containing an enumerable of all the current suspension reasons for a subscription. It is possible for a subscription to have many concurrent, overlapping suspension reasons. A subscription&#39;s STATUS is SUSPENDED until all pending suspensions are removed.
...
- OTHER - The customer is suspended for an internal Google reason (e.g. abuse or
otherwise).
