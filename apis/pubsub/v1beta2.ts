    ...
    subscriptions: {
      ...
      acknowledge(params, options, callback) {
        ...
        return createAPIRequest(parameters, callback);
          ...
          * subscription on the same project as the topic. Note that for REST
          * API requests, you must specify a name.
 ...
 * @memberOf! pubsub(v1beta2)
 * @type object
 * @property {pubsub(v1beta2).Binding[]} bindings Associates a list of `members` to a `role`. `bindings` with no members will result in an error.
 ...
 * @property {string} data The message payload. For JSON requests, the value of this field must be [base64-encoded](https://tools.ietf.org/html/rfc4648).
