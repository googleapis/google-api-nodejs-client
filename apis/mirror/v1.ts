...
- POST_AN_UPDATE - Shares a timeline item with the transcription of user speech
from the &quot;Post an update&quot; voice menu command.
 */
/**
 * @typedef Contact
 * @memberOf! mirror(v1)
...
userActions.type set to CUSTOM and the userActions.payload set to the ID of this
menu item. This is the default value.
- Built-in actions:
...
is not set, read the text field; if none of those fields are set, this menu item
is ignored.
...
- When the action is GET_MEDIA_INPUT, the payload is the text transcription of a
user&#39;s speech input
...
- PENDING - Value shown when the menuItem has been selected by the user but can
still be cancelled.
...
- DEFAULT - Notifications of default importance. A chime will be played to alert
users.
 */
/**
 * @typedef Setting
...
- timezone - The key to the user’s current time zone region as defined in the tz
...
- cid: &lt;img src=&quot;cid:attachment_id&quot;&gt; where attachment_id is the
...
If an item is marked as a bundle cover, it will be the entry point to the bundle
of items that have the same bundleId as that item. It will be shown only on the
main timeline — not within the opened bundle.
...
Glassware are encouraged to populate this field for every timeline item, even if
the item does not contain speakableText or text so that the user can learn the
...
- LAUNCH - the user initiated a voice command.  In the future, additional types
may be added. UserActions with unrecognized types should be ignored.
 */
/**
 * @typedef UserData
 * @memberOf! mirror(v1)
