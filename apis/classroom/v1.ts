  ...
  self.courses =
      {
        ...
        get(params, options, callback) {
          ...
          return createAPIRequest(parameters, callback);
            ...
            * Returned courses are ordered by creation time, with the most
            * recently created coming first.  This method returns the following
        ...
        list(params, options, callback) {
          ...
          return createAPIRequest(parameters, callback);
            ...
            * requesting user is not permitted to modify the requested course or
            * for access errors. * `NOT_FOUND` if no course exists with the
            * requested ID. * `INVALID_ARGUMENT` if invalid fields are specified
            * in the update mask or if no update mask is supplied. *
  ...
  self.invitations = {
    ...
    accept(params, options, callback) {
      ...
      return createAPIRequest(parameters, callback);
        ...
        * may exist at a time. Delete and re-create an invitation to make
        * changes.  This method returns the following error codes:  *
        * `PERMISSION_DENIED` if the requesting user is not permitted to create
        * invitations for this course or for access errors. * `NOT_FOUND` if the
        * course or the user does not exist. * `FAILED_PRECONDITION` if the
    ...
    create(params, options, callback) {
      ...
      return createAPIRequest(parameters, callback);
        ...
        * to delete the requested invitation or for access errors. * `NOT_FOUND`
    ...
    guardianInvitations: {
      ...
      create(params, options, callback) {
        ...
        return createAPIRequest(parameters, callback);
          ...
          * the following error codes:  * `PERMISSION_DENIED` if the requesting
          * user is not permitted to view   guardian invitations for the student
          * identified by the `student_id`, if   guardians are not enabled for
          * the domain in question, or for other   access errors. *
          * `INVALID_ARGUMENT` if a `student_id` is specified, but its format
          * cannot   be recognized (it is not an email address, nor a
          * `student_id` from the   API, nor the literal string `me`). *
          * `NOT_FOUND` if Classroom cannot find any record of the given student
          * or   `invitation_id`. May also be returned if the student exists,
          * but the   requesting user does not have access to see that student.
      ...
      get(params, options, callback) {
        ...
        return createAPIRequest(parameters, callback);
          ...
          * user is permitted to view, filtered by the parameters provided.
          * This method returns the following error codes:  *
          * `PERMISSION_DENIED` if a `student_id` is specified, and the
          * requesting   user is not permitted to view guardian invitations for
          * that student, if   `"-"` is specified as the `student_id` and the
          * user is not a domain   administrator, if guardians are not enabled
          * for the domain in question,   or for other access errors. *
          * `INVALID_ARGUMENT` if a `student_id` is specified, but its format
          * cannot   be recognized (it is not an email address, nor a
          * `student_id` from the   API, nor the literal string `me`). May also
      ...
      list(params, options, callback) {
        ...
        return createAPIRequest(parameters, callback);
          ...
          * access errors. * `FAILED_PRECONDITION` if the guardian link is not
          * in the `PENDING` state. * `INVALID_ARGUMENT` if the format of the
          * student ID provided   cannot be recognized (it is not an email
          ...
          * provided is a valid student ID, but   Classroom has no record of
          * that student, or if the `id` field does not   refer to a guardian
    ...
    guardians: {
      ...
      delete (params, options, callback) {
        ...
        return createAPIRequest(parameters, callback);
          ...
          * following error codes:  * `PERMISSION_DENIED` if no user that
          * matches the provided `student_id`   is visible to the requesting
          * user, if the requesting user is not   permitted to view guardian
          * information for the student identified by the   `student_id`, if
          * guardians are not enabled for the domain in question,   or for other
          * access errors. * `INVALID_ARGUMENT` if a `student_id` is specified,
          * but its format cannot   be recognized (it is not an email address,
          * nor a `student_id` from the   API, nor the literal string `me`). *
          * `NOT_FOUND` if the requesting user is permitted to view guardians
          * for   the requested `student_id`, but no `Guardian` record exists
          * for that   student that matches the provided `guardian_id`.
      ...
      get(params, options, callback) {
        ...
        return createAPIRequest(parameters, callback);
          ...
          * This method returns the following error codes:  *
          * `PERMISSION_DENIED` if a `student_id` is specified, and the
          * requesting   user is not permitted to view guardian information for
          * that student, if   `"-"` is specified as the `student_id` and the
          * user is not a domain   administrator, if guardians are not enabled
          * for the domain in question,   if the `invited_email_address` filter
          * is set by a user who is not a   domain administrator, or for other
          * access errors. * `INVALID_ARGUMENT` if a `student_id` is specified,
          * but its format cannot   be recognized (it is not an email address,
          * nor a `student_id` from the   API, nor the literal string `me`). May
          * also be returned if an invalid   `page_token` is provided. *
          * `NOT_FOUND` if a `student_id` is specified, and its format can be
          * recognized, but Classroom has no record of that student.
 ...
 * @memberOf! classroom(v1)
 * @type object
 * @property {string} alternateLink Absolute link to this announcement in the Classroom web UI. This is only populated if `state` is `PUBLISHED`.  Read-only.
 * @property {string} assigneeMode Assignee mode of the announcement. If unspecified, the default value is `ALL_STUDENTS`.
 * @property {string} courseId Identifier of the course.  Read-only.
 * @property {string} creationTime Timestamp when this announcement was created.  Read-only.
 * @property {string} creatorUserId Identifier for the user that created the announcement.  Read-only.
 * @property {string} id Classroom-assigned identifier of this announcement, unique per course.  Read-only.
 * @property {classroom(v1).IndividualStudentsOptions} individualStudentsOptions Identifiers of students with access to the announcement. This field is set only if `assigneeMode` is `INDIVIDUAL_STUDENTS`. If the `assigneeMode` is `INDIVIDUAL_STUDENTS`, then only students specified in this field will be able to see the announcement.
 * @property {classroom(v1).Material[]} materials Additional materials.  Announcements must have no more than 20 material items.
 * @property {string} scheduledTime Optional timestamp when this announcement is scheduled to be published.
 * @property {string} state Status of this announcement. If unspecified, the default state is `DRAFT`.
 * @property {string} text Description of this announcement. The text must be a valid UTF-8 string containing no more than 30,000 characters.
 * @property {string} updateTime Timestamp of the most recent change to this announcement.  Read-only.
 */
/**
 * @typedef Assignment
 * @memberOf! classroom(v1)
 * @type object
 * @property {classroom(v1).DriveFolder} studentWorkFolder Drive folder where attachments from student submissions are placed. This is only populated for course teachers and administrators.
 */
/**
 ...
 * @memberOf! classroom(v1)
 * @type object
 * @property {string} topicName The `name` field of a Cloud Pub/Sub [Topic](https://cloud.google.com/pubsub/docs/reference/rest/v1/projects.topics#Topic).
 */
/**
 * @typedef Course
 * @memberOf! classroom(v1)
 ...
 * @property {string} calendarId The Calendar ID for a calendar that all course members can see, to which Classroom adds events for course work and announcements in the course.  Read-only.
 ...
 * @memberOf! classroom(v1)
 * @type object
 * @property {string} alias Alias string. The format of the string indicates the desired alias scoping.  * `d:&lt;name&gt;` indicates a domain-scoped alias.   Example: `d:math_101` * `p:&lt;name&gt;` indicates a project-scoped alias.   Example: `p:abc123`  This field has a maximum length of 256 characters.
 */
/**
 ...
 * @property {string} assigneeMode Assignee mode of the coursework. If unspecified, the default value is `ALL_STUDENTS`.
 ...
 * @property {string} creatorUserId Identifier for the user that created the coursework.  Read-only.
 ...
 * @property {classroom(v1).IndividualStudentsOptions} individualStudentsOptions Identifiers of students with access to the coursework. This field is set only if `assigneeMode` is `INDIVIDUAL_STUDENTS`. If the `assigneeMode` is `INDIVIDUAL_STUDENTS`, then only students specified in this field will be assigned the coursework.
 ...
 * @memberOf! classroom(v1)
 * @type object
 * @property {classroom(v1).CourseRosterChangesInfo} courseRosterChangesInfo Information about a `Feed` with a `feed_type` of `COURSE_ROSTER_CHANGES`. This field must be specified if `feed_type` is `COURSE_ROSTER_CHANGES`.
 ...
 * @property {number} maxPoints The denominator of the grade at this time in the submission grade history.
 ...
 * @memberOf! classroom(v1)
 * @type object
 * @property {string[]} studentIds Identifiers for the students that have access to the coursework/announcement.
 */
/**
 * @typedef Invitation
 * @memberOf! classroom(v1)
 ...
 * @memberOf! classroom(v1)
 * @type object
 * @property {string} assigneeMode Mode of the announcement describing whether it will be accessible by all students or specified individual students.
 * @property {classroom(v1).ModifyIndividualStudentsOptions} modifyIndividualStudentsOptions Set which students can view or cannot view the announcement. Must be specified only when `assigneeMode` is `INDIVIDUAL_STUDENTS`.
 */
/**
 ...
 * @memberOf! classroom(v1)
 * @type object
 * @property {string} assigneeMode Mode of the coursework describing whether it will be assigned to all students or specified individual students.
 * @property {classroom(v1).ModifyIndividualStudentsOptions} modifyIndividualStudentsOptions Set which students are assigned or not assigned to the coursework. Must be specified only when `assigneeMode` is `INDIVIDUAL_STUDENTS`.
 */
/**
 * @typedef ModifyIndividualStudentsOptions
 * @memberOf! classroom(v1)
 * @type object
 * @property {string[]} addStudentIds Ids of students to be added as having access to this coursework/announcement.
 * @property {string[]} removeStudentIds Ids of students to be removed from having access to this coursework/announcement.
 */
/**
 ...
 * @property {string} expiryTime The time until which the `Registration` is effective.  This is a read-only field assigned by the server.
 * @property {classroom(v1).Feed} feed Specification for the class of notifications that Classroom should deliver to the `destination`.
 * @property {string} registrationId A server-generated unique identifier for this `Registration`.  Read-only.
 */
/**
 ...
 * @property {number} assignedGrade Optional grade. If unset, no grade was set. This value must be non-negative. Decimal (i.e. non-integer) values are allowed, but will be rounded to two decimal places.  This may be modified only by course teachers.
 * @property {classroom(v1).AssignmentSubmission} assignmentSubmission Submission content when course_work_type is ASSIGNMENT.  Students can modify this content using google.classroom.Work.ModifyAttachments.
 ...
 * @property {number} draftGrade Optional pending grade. If unset, no grade was set. This value must be non-negative. Decimal (i.e. non-integer) values are allowed, but will be rounded to two decimal places.  This is only visible to and modifiable by course teachers.
 ...
 * @property {classroom(v1).SubmissionHistory[]} submissionHistory The history of the submission (includes state and grade histories).  Read-only.
 ...
 * @property {boolean} verifiedTeacher Represents whether a G Suite for Education user&#39;s domain administrator has explicitly verified them as being a teacher. If the user is not a member of a G Suite for Education domain, than this field will always be false.  Read-only
 */
/**
