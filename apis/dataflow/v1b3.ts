 ...
 * @property {dataflow(v1b3).StructuredMessage} description A message describing why the system decided to adjust the current number of workers, why it failed, or why the system decided to not make any changes to the number of workers.
 ...
 * @property {string} time The time this event was emitted to indicate a new target or current num_workers value.
 */
/**
 ...
 * @property {string} originalTransformOrCollection User name for the original user transform or collection with which this source is most closely associated.
 ...
 * @type object
 * @property {string} name Dataflow service generated name for this source.
 * @property {string} originalTransform User name for the original user transform with which this transform is most closely associated.
 * @property {string} userName Human-readable name for this transform; may be user or system generated.
 */
/**
 ...
 * @property {integer} inputIndex Index of an input collection that&#39;s being read from/written to as a side input. The index identifies a step&#39;s side inputs starting by 1 (e.g. the first side input has input_index 1, the third has input_index 3). Side inputs are identified by a pair of (original_step_name, input_index). This field helps uniquely identify them.
 ...
 * @property {string} originalRequestingStepName The step name requesting an operation, such as GBK. I.e. the ParDo causing a read/write from shuffle to occur, or a read from side inputs.
 ...
 * @memberOf! dataflow(v1b3)
 * @type object
 * @property {number} rate Average CPU utilization rate (% non-idle cpu / second) since previous sample.
 * @property {string} timestamp Timestamp of the measurement.
 * @property {string} totalMs Total active CPU time across all cores (ie., non-idle) in milliseconds since start-up.
 */
/**
 ...
 * @property {string} gcsPath Required. A Cloud Storage path to the template from which to create the job. Must be a valid Cloud Storage URL, beginning with `gs://`.
 ...
 * @property {string} key The key identifying the display data. This is intended to be used as a label for the display data when viewed in a dax monitoring system.
 * @property {string} label An optional label to display in a dax UI for the element.
 * @property {string} namespace The namespace for the key. This is usually a class name or programming language namespace (i.e. python module) which defines the display data. This allows a dax monitoring system to specially handle the data and perform custom rendering.
 * @property {string} shortStrValue A possible additional shorter value to display. For example a java_class_name_value of com.mypackage.MyDoFn will be stored with MyDoFn as the short_str_value and com.mypackage.MyDoFn as the java_class_name value. short_str_value can be displayed and java_class_name_value will be displayed as a tooltip.
 ...
 * @property {dataflow(v1b3).SplitInt64} sum Use an int64 since we&#39;d prefer the added precision. If overflow is a common problem we can detect it and use an additional int64 or a double.
 ...
 * @property {object} sdkPipelineOptions The Cloud Dataflow SDK pipeline options specified by the user. These options are passed through the service and are used to recreate the SDK pipeline options on the worker in a language agnostic and platform independent way.
 ...
 * @property {dataflow(v1b3).WorkerPool[]} workerPools The worker pools. At least one &quot;harness&quot; worker pool must be specified in order for the job to have workers.
 */
/**
 ...
 * @memberOf! dataflow(v1b3)
 * @type object
 * @property {dataflow(v1b3).TemplateMetadata} metadata The template metadata describing the template name, available parameters, etc.
 * @property {dataflow(v1b3).Status} status The status of the get template request. Any problems with the request will be indicated in the error_details.
 */
/**
 * @typedef Histogram
 * @memberOf! dataflow(v1b3)
 * @type object
 * @property {string[]} bucketCounts Counts of values in each bucket. For efficiency, prefix and trailing buckets with count = 0 are elided. Buckets can store the full range of values of an unsigned long, with ULLONG_MAX falling into the 59th bucket with range [1e19, 2e19).
 * @property {integer} firstBucketOffset Starting index of first stored bucket. The non-inclusive upper-bound of the ith bucket is given by:   pow(10,(i-first_bucket_offset)/3) * (1,2,5)[(i-first_bucket_offset)%3]
 */
/**
 ...
 * @property {string} createTime The timestamp when the job was initially created. Immutable and set by the Cloud Dataflow service.
 ...
 * @property {string} id The unique ID of this job.  This field is set by the Cloud Dataflow service when the Job is created, and is immutable for the life of the job.
 ...
 * @property {string} name The user-specified Cloud Dataflow job name.  Only one Job with a given name may exist in a project at any given time. If a caller attempts to create a Job with the same name as an already-existing Job, the attempt returns the existing Job.  The name must match the regular expression `[a-z]([-a-z0-9]{0,38}[a-z0-9])?`
 * @property {dataflow(v1b3).PipelineDescription} pipelineDescription Preliminary field: The format of this data may change at any time. A description of the user pipeline and stages through which it is executed. Created by Cloud Dataflow service.  Only retrieved with JOB_VIEW_DESCRIPTION or JOB_VIEW_ALL.
 * @property {string} projectId The ID of the Cloud Platform project that the job belongs to.
 * @property {string} replacedByJobId If another job is an update of this job (and thus, this job is in `JOB_STATE_UPDATED`), this field contains the ID of that job.
 * @property {string} replaceJobId If this job is an update of an existing job, this field is the job ID of the job it replaced.  When sending a `CreateJobRequest`, you can update a job by specifying it here. The job named here is stopped, and its intermediate state is transferred to this job.
 * @property {string} requestedState The job&#39;s requested state.  `UpdateJob` may be used to switch between the `JOB_STATE_STOPPED` and `JOB_STATE_RUNNING` states, by setting requested_state.  `UpdateJob` may also be used to directly set a job&#39;s requested state to `JOB_STATE_CANCELLED` or `JOB_STATE_DONE`, irrevocably terminating the job if it has not already reached a terminal state.
 * @property {dataflow(v1b3).ExecutionStageState[]} stageStates This field may be mutated by the Cloud Dataflow service; callers cannot mutate it.
 ...
 * @property {object} transformNameMapping The map of transform name prefixes of the job to be replaced to the corresponding name prefixes of the new job.
 ...
 * @property {string} deprecatedPersistentDirectory DEPRECATED. The location of the persistent state for this range, as a persistent directory in the worker local filesystem.
 * @property {string} end The end (exclusive) of the key range.
 * @property {string} start The start (inclusive) of the key range.
 */
/**
 ...
 * @memberOf! dataflow(v1b3)
 * @type object
 * @property {dataflow(v1b3).Job} job The job that was launched, if the request was not a dry run and the job was successfully launched.
 */
/**
 ...
 * @property {any} distribution A struct value describing properties of a distribution of numeric values.
 * @property {any} internal Worker-computed aggregate value for internal use by the Dataflow service.
 * @property {string} kind Metric aggregation kind.  The possible metric aggregation kinds are &quot;Sum&quot;, &quot;Max&quot;, &quot;Min&quot;, &quot;Mean&quot;, &quot;Set&quot;, &quot;And&quot;, &quot;Or&quot;, and &quot;Distribution&quot;. The specified aggregation kind is case-insensitive.  If omitted, this is not an aggregated value but instead a single metric sample value.
 ...
 * @property {string} subscription A pubsub subscription, in the form of &quot;pubsub.googleapis.com/subscriptions/&lt;project-id&gt;/&lt;subscription-name&gt;&quot;
 ...
 * @property {string} topic A pubsub topic, in the form of &quot;pubsub.googleapis.com/topics/&lt;project-id&gt;/&lt;topic-name&gt;&quot;
 ...
 * @property {string} originalName System-defined name for the Read instruction for this source in the original workflow graph.
 * @property {dataflow(v1b3).SourceSplitRequest} split Information about a request to split a source.
 * @property {string} stageName System-defined name of the stage containing the source operation. Unique across the workflow.
 * @property {string} systemName System-defined name of the Read instruction for this source. Unique across the workflow.
 */
/**
 ...
 * @property {string} name The name that identifies the step. This must be unique for each step with respect to all other steps in the Cloud Dataflow job.
 * @property {object} properties Named properties associated with the step. Each kind of predefined step has its own required set of properties. Must be provided on Create.  Only retrieved with JOB_VIEW_ALL.
 */
/**
 ...
 * @property {string} windmillServiceEndpoint If present, the worker must use this endpoint to communicate with Windmill Service dispatchers, otherwise the worker must continue to use whatever endpoint it had been using.
 * @property {string} windmillServicePort If present, the worker must use this port to communicate with Windmill Service dispatchers. Only applicable when windmill_service_endpoint is specified.
 */
/**
 ...
 * @memberOf! dataflow(v1b3)
 * @type object
 * @property {string} messageKey Idenfier for this message type.  Used by external systems to internationalize or personalize message.
 ...
 * @property {boolean} logToSerialconsole Whether to send taskrunner log info to Google Compute Engine VM serial console.
 ...
 * @property {string[]} oauthScopes The OAuth2 scopes to be requested by the taskrunner in order to access the Cloud Dataflow API.
 ...
 * @property {string} workerHarnessContainerImage Required. Docker container image that executes the Cloud Dataflow worker harness, residing in Google Container Registry.
 ...
 * @property {string} servicePath The Cloud Dataflow service path relative to the root URL, for example, &quot;dataflow/v1b3/projects&quot;.
 ...
 * @memberOf! dataflow(v1b3)
 * @type object
 * @property {string} reason The reason for the worker shutdown. Current possible values are:   &quot;UNKNOWN&quot;: shutdown reason is unknown.   &quot;PREEMPTION&quot;: shutdown reason is preemption. Other possible reasons may be added in the future.
 */
/**
