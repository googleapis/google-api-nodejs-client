 ...
 * var deploymentmanager = google.deploymentmanager('alpha');
 *
 ...
 * @memberOf! deploymentmanager(alpha)
 * @type object
 * @property {deploymentmanager(alpha).BasicAuth} basicAuth Basic Auth Credentials for this Type.
 */
/**
 * @typedef Deployment
 * @memberOf! deploymentmanager(alpha)
...
* @property {deploymentmanager(alpha).Binding[]} bindings Associates a list of `members` to a `role`. Multiple `bindings` must not be specified for the same `role`. `bindings` with no members will result in an error.
 ...
 * @property {deploymentmanager(alpha).Condition[]} conditions Additional restrictions that must be met
