...
function Toolresults(options) {
  ...
  self.projects = {
    ...
    , /**
       ...
       * calling this method in parallel from multiple clients will only create
       * a single bucket. In order to avoid unnecessary storage charges, the
       * bucket is configured to automatically delete objects older than 90
       * days.  The bucket is created with the following permissions: - Owner
       * access for owners of central storage project (FTL-owned) - Writer
       * access for owners/editors of customer project - Reader access for
       * viewers of customer project The default ACL on objects created in the
       * bucket is: - Owner access for owners of central storage project -
       * Reader access for owners/editors/viewers of customer project See Google
       * Cloud Storage documentation for more details.  If there is already a
       * default bucket set and the project can access the bucket, this call
       * does nothing. However, if the project doesn't have the permission to
       * access the bucket or the bucket is deleted, a new bucket will be
       * created.  May return any canonical error codes, including the
       * following:  - PERMISSION_DENIED - if the user is not authorized to
       * write to project - Any error code raised by Google Cloud Storage
    ...
    histories: {
      ...
      , /**
         ...
         * used to order the history with the same modification time.  May
      ...
      executions: {
        ...
        , /**
           ...
           * @desc Gets an Execution.  May return any of the following canonical
           * error codes:  - PERMISSION_DENIED - if the user is not authorized
           * to write to project - INVALID_ARGUMENT - if the request is
        ...
        , /**
           ...
           * sorted by creation_time in descending order. The execution_id key
           * will be used to order the executions with the same creation_time.
           * May return any of the following canonical error codes:  -
           * PERMISSION_DENIED - if the user is not authorized to read project -
           * INVALID_ARGUMENT - if the request is malformed - NOT_FOUND - if the
           * containing History does not exist
        ...
        clusters: {
          ...
          , /**
             ...
             * created after the execution is finished. Clusters are created
             * from a set of screenshots. Between any two screenshots, a
             * matching score is calculated based off their metadata that
             * determines how similar they are. Screenshots are placed in the
        ...
        steps:
            {
               ...
               * @desc Creates a Step.  The returned Step will have the id set.
               * May return any of the following canonical error codes:  -
               * PERMISSION_DENIED - if the user is not authorized to write to
               * project - INVALID_ARGUMENT - if the request is malformed -
               * FAILED_PRECONDITION - if the step is too large (more than
              ...
              , /**
                 ...
                 * error codes:  - PERMISSION_DENIED - if the user is not
                 * authorized to read project - INVALID_ARGUMENT - if the
              ...
              , /**
                 ...
                 * following error code(s): - NOT_FOUND - The specified
              ...
              , /**
                 ...
                 * @desc Lists Steps for a given Execution.  The steps are
                 * sorted by creation_time in descending order. The step_id key
                 * will be used to order the steps with the same creation_time.
                 * May return any of the following canonical error codes:  -
                 * PERMISSION_DENIED - if the user is not authorized to read
                 * project - INVALID_ARGUMENT - if the request is malformed -
                 * FAILED_PRECONDITION - if an argument in the request happens
                 * to be invalid; e.g. if an attempt is made to list the
                 * children of a nonexistent Step - NOT_FOUND - if the
              ...
              , /**
                 ...
                 * @desc Updates an existing Step with the supplied partial
                 * entity.  May return any of the following canonical error
                 * codes:  - PERMISSION_DENIED - if the user is not authorized
                 * to write project - INVALID_ARGUMENT - if the request is
                 * malformed - FAILED_PRECONDITION - if the requested state
                 * transition is illegal (e.g try to upload a duplicate xml
                 * file), if the updated step is too large (more than 10Mib) -
              ...
              , /**
                 ...
                 * @desc Publish xml files to an existing Step.  May return any
                 * of the following canonical error codes:  - PERMISSION_DENIED
                 * - if the user is not authorized to write project -
                 * INVALID_ARGUMENT - if the request is malformed -
                 * FAILED_PRECONDITION - if the requested state transition is
                 * illegal, e.g try to upload a duplicate xml file or a file too
                 * large. - NOT_FOUND - if the containing Execution does not
                 * exist
                 * @alias
                 * toolresults.projects.histories.executions.steps.publishXunitXmlFiles
                 * @memberOf! toolresults(v1beta3)
              ...
              perfMetricsSummary: {
                 ...
                 * @desc Creates a PerfMetricsSummary resource. Returns the
                 * existing one if it has already been created.  May return any
                 * of the following error code(s): - NOT_FOUND - The containing
                 * Step does not exist
                 * @alias
                 * toolresults.projects.histories.executions.steps.perfMetricsSummary.create
                 * @memberOf! toolresults(v1beta3)
              ...
              perfSampleSeries: {
                 ...
                 * already exists for the given Step - NOT_FOUND - The
                ...
                , /**
                   ...
                   * provides an optional filter which specifies one or more
                   * PerfMetricsType to include in the result; if none returns
                   * all. The resulting PerfSampleSeries are sorted by ids.  May
                   * return any of the following canonical error codes: -
                ...
                samples:
                    {
                       ...
                       * @desc Creates a batch of PerfSamples - a client can
                       * submit multiple batches of Perf Samples through
                       * repeated calls to this method in order to split up a
                       * large request payload - duplicates and existing
                       * timestamp entries will be ignored. - the batch
                       * operation may partially succeed - the set of elements
                       * successfully inserted is returned in the response
                       * (omits items which already existed in the database).
                       * May return any of the following canonical error codes:
                       * - NOT_FOUND - The containing PerfSampleSeries does not
                       * exist
                       * @alias
                       * toolresults.projects.histories.executions.steps.perfSampleSeries.samples.batchCreate
                       * @memberOf! toolresults(v1beta3)
                      ...
                      , /**
                         ...
                         * Series - The list results are sorted by timestamps
                         * ascending - The default page size is 500 samples; and
                         * maximum size allowed 5000 - The response token
                         * indicates the last returned PerfSample timestamp -
                         * When the results size exceeds the page size, submit a
                         * subsequent request including the page token to return
                         * the rest of the samples up to the page limit  May
                         * return any of the following canonical error codes: -
                         * OUT_OF_RANGE - The specified request page_token is
                         * out of valid range - NOT_FOUND - The containing
                         * PerfSampleSeries does not exist
                         * @alias
                         * toolresults.projects.histories.executions.steps.perfSampleSeries.samples.list
                         * @memberOf! toolresults(v1beta3)
              ...
              thumbnails: {
                 ...
                 * @desc Lists thumbnails of images attached to a step.  May
                 * return any of the following canonical error codes: -
                 * PERMISSION_DENIED - if the user is not authorized to read
                 * from the project, or from any of the images -
                 * INVALID_ARGUMENT - if the request is malformed - NOT_FOUND -
                 * if the step does not exist, or if any of the images do not
                 * exist
                 * @alias
                 * toolresults.projects.histories.executions.steps.thumbnails.list
                 * @memberOf! toolresults(v1beta3)
 ...
 * @memberOf! toolresults(v1beta3)
 * @type object
 * @property {string} typeUrl A URL/resource name that uniquely identifies the type of the serialized protocol buffer message. The last segment of the URL&#39;s path must represent the fully qualified name of the type (as in `path/google.protobuf.Duration`). The name should be in a canonical form (e.g., leading &quot;.&quot; is not accepted).
...
precompiled into a binary to avoid any lookup. Therefore, binary compatibility
needs to be preserved on changes to types. (Use versioned type names to manage
breaking changes.)
...
If the state is set to COMPLETE, all the in-progress steps within the execution
will be set as COMPLETE. If the outcome of the step is not set, the outcome will
be set to INCONCLUSIVE.

- In response always set - In create/update request: optional
 * @property {string} testExecutionMatrixId TestExecution Matrix ID that the TestExecutionService uses.

- In response: present if set by create - In create: optional - In update: never
set
 */
/**
...
Tokens are valid for up to one hour from the time of the first list request. For
instance, if you make a list request at 1PM and use the token from this first
request 10 minutes later, the token from this second response will only be valid
for 50 minutes.
 */
/**
...
If set, indicates that there are more steps to read, by calling list again with
...
If set, indicates that there are more thumbnails to read, by calling list again
with this value in the page_token field.
 * @property {toolresults(v1beta3).Image[]} thumbnails A list of image data.

Images are returned in a deterministic order; they are ordered by these factors,
in order of importance: * First, by their associated test case. Images without a
test case are considered greater than images with one. * Second, by their
creation time. Images without a creation time are greater than images with one.
...
This is the device usage used for billing purpose, which is different from the
run_duration, for example, infrastructure failure won&#39;t be charged for
device usage.

PRECONDITION_FAILED will be returned if one attempts to set a device_usage on a
step which already has this field set.
...
`dimension_value = [&#39;attempt&#39;: &#39;1&#39;, &#39;device&#39;: &#39;Nexus
6&#39;]`
...
`dimension_value = [&#39;attempt&#39;: &#39;&#39;, &#39;device&#39;: &#39;Nexus
6&#39;]`
...
A PRECONDITION_FAILED will be returned if another step in this execution already
has the same name and dimension_value, but differs on other data fields, for
example, step field is different.
...
optional; any new key/value pair will be added to the map, and any new value for
...
A PRECONDITION_FAILED will be returned upon creating a new step if it shares its
name and dimension_value with an existing step. If two steps represent a similar
action, but have different dimension values, they should share the same name.
For instance, if the same set of tests is run on two different platforms, the
...
completion_time when the step is set to the COMPLETE state. In some cases, it is
appropriate to set this value separately: For instance, if a step is created,
but the operation it represents is queued for a few minutes before it executes,
it would be appropriate not to include the time spent queued in its
run_duration.

PRECONDITION_FAILED will be returned if one attempts to set a run_duration on a
step which already has this field set.

- In response: present if previously set; always present on COMPLETE step - In
create request: optional - In update request: optional
...
set to COMPLETE once. A PRECONDITION_FAILED will be returned if the state is set
...
That is, the bytes here are precisely the bytes that make up the thumbnail file;
they can be served as an image as-is (with the appropriate content type.)
